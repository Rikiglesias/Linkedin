...
export function renderKpiComparison(trend) {
    if (trend.length === 0) {
...
export function renderPredictiveRisk(predictive) {
...
export function renderOperationalSlo(slo) {
    if (!slo) {
    ...
    setText('slo-current-status', `SLO corrente: ${slo.status} (queueLag ${Math.round(current.queueLagSeconds ?? 0)}s, running ${Math.round(current.oldestRunningJobSeconds ?? 0)}s)`);
    const window7d = (slo.windows ?? []).find((row) => row.windowDays === 7);
    const window30d = (slo.windows ?? []).find((row) => row.windowDays === 30);
    setText('slo-7d-status', window7d
        ...
        : 'SLO 7d: —');
    setText('slo-30d-status', window30d
...
export function renderSelectorCacheKpi(kpi) {
    if (!kpi) {
...
export function renderReviewQueue(reviewQueue) {
...
export function renderCommentSuggestions(queue) {
...
export function renderAbLeaderboard(rows) {
...
export function renderTimingSlots(slots) {
...
export function renderTimeline(entries, optionSets) {
