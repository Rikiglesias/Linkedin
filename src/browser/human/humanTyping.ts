 ...
 * ─────────────────────────────────────────────────────────────────
 * Digitazione umana carattere-per-carattere: humanType (typo + correzione, word-flow multiplier,
 * micro-pause distrazione), digitaTestoUmano, e la pressione dei tasti speciali. Estratto da
 * humanBehavior.ts (A13, split SRP).
 *
 * LA CURVA vive in `keystrokeTiming.ts` (split 2026-08-05, oltre le 300 righe): dwell, flight e
 * rallentamento per testi lunghi sono formule pure e non hanno bisogno di un `Locator`. Qui restano
 * i CICLI che le usano per scrivere su una superficie. Lo split e' stato solo uno SPOSTAMENTO:
 * nessuna formula e nessuna costante e' stata riscritta.
 *
 * TIMING-CORE (keystroke dynamics) anche qui: l'ORDINE delle operazioni — dwell come `delay` di
 * Playwright, flight atteso a parte — e le probabilità di correzione/distrazione sono VERBATIM.
...
import { fattoreLunghezzaTesto, humanKeystrokeDelayMs, humanKeystrokeDwellMs } from './keystrokeTiming';
...
export async function humanType(
    ...
    // 🔴 Era una copia A MANO della stessa curva che `fattoreLunghezzaTesto` gia' definiva, e che
    // `digitaTestoUmano` gia' chiamava: due definizioni della stessa cadenza, cioe' esattamente cio'
    // che il commento di quella funzione dice di evitare («due copie divergerebbero al primo
    // ritocco ⇒ superfici con ritmi diversi»). Reso visibile dallo split, chiuso qui: stessi
    // scaglioni, stessi valori, una sola sorgente.
    const lengthSlowFactor = fattoreLunghezzaTesto(text);
