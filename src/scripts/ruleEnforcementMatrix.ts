...
function buildMatrix(): RuleResult[] {
    ...
    const results: RuleResult[] = [
        ...
        hookRule(
            'secrets-pre-edit',
            'Blocco scrittura segreti e file sensibili',
            'Hook bloccante',
            'hook-bloccante',
            hookExists(settings, 'PreToolUse', 'pre-edit-secrets.ps1'),
            'PreToolUse Edit/Write → pre-edit-secrets.ps1',
            'Aggiungere pre-edit-secrets.ps1 in PreToolUse Edit/Write',
        ),
        hookRule(
            'mcp-guard',
            'Guardia MCP per operazioni distruttive o ad alto impatto',
            'Hook bloccante',
            'hook-bloccante',
            hookExists(settings, 'PreToolUse', 'pre-mcp-guard.ps1'),
            'PreToolUse mcp__.* → pre-mcp-guard.ps1',
            'Aggiungere pre-mcp-guard.ps1 in PreToolUse mcp__.*',
        ),
        ...
        hookRule(
            ...
            'Recap e blast radius prima di task multi-file',
            'Advisory hook',
            'advisory-hook',
            hookExists(settings, 'UserPromptSubmit', 'multi-file-recap-check.ps1'),
            'UserPromptSubmit → multi-file-recap-check.ps1',
            'Aggiungere multi-file-recap-check.ps1 in UserPromptSubmit',
        ),
        hookRule(
            'verify-intent-hook',
            'Verifica codice reale prima di fix/correzioni',
            'Advisory hook',
            'advisory-hook',
            hookExists(settings, 'UserPromptSubmit', 'pre-edit-verify-intent.ps1'),
            'UserPromptSubmit → pre-edit-verify-intent.ps1',
            'Aggiungere pre-edit-verify-intent.ps1 in UserPromptSubmit',
        ),
        hookRule(
            'best-practice-pre-edit',
            'Best practice e web policy per tipo file prima di edit',
            'Advisory hook',
            'advisory-hook',
            hookExists(settings, 'PreToolUse', 'pre-edit-best-practice.ps1'),
            'PreToolUse Edit/Write → pre-edit-best-practice.ps1',
            'Aggiungere pre-edit-best-practice.ps1 in PreToolUse Edit/Write',
        ),
        hookRule(
        ...
        hookRule(
            'compact-handoff-hook',
            'Handoff operativo prima della compattazione contesto',
            'Advisory hook',
            'advisory-hook',
            hookExists(settings, 'PreCompact', 'pre-compact-handoff.ps1'),
            'PreCompact → pre-compact-handoff.ps1',
            'Aggiungere pre-compact-handoff.ps1 in PreCompact',
        ),
        hookRule(
            'quality-log',
        ...
        hookRule(
            ...
            'Commit/push via trigger solo dopo gate',
            'Hook asincrono',
            'hook-asincrono',
            hookExists(settings, 'PostToolUse', 'post-edit-request-action.ps1'),
            'PostToolUse Edit/Write → post-edit-request-action.ps1 con gate verificati da audit:hooks',
            'Aggiungere post-edit-request-action.ps1 o rimuovere trigger REQUEST_COMMIT/REQUEST_PUSH dai workflow',
        ),
        hookRule(
            'websearch-log',
            'Log uso WebSearch per verificabilita fonte',
            'Hook asincrono',
            'hook-asincrono',
            hookExists(settings, 'PostToolUse', 'post-websearch-log.ps1'),
            'PostToolUse WebSearch → post-websearch-log.ps1',
            'Aggiungere post-websearch-log.ps1 in PostToolUse WebSearch',
        ),
        hookRule(
            'file-size-check',
        ...
        hookRule(
            'subagent-stop-log',
            'Log output subagent a fine delega',
            'Hook asincrono',
            'hook-asincrono',
            hookExists(settings, 'SubagentStop', 'subagent-stop.ps1'),
            'SubagentStop → subagent-stop.ps1',
            'Aggiungere subagent-stop.ps1 in SubagentStop',
        ),
        hookRule(
            'stop-commit-gate',
            'Avviso di chiusura sessione con working tree dirty',
            'Advisory hook',
            'advisory-hook',
            hookExists(settings, 'Stop', 'pre-stop-commit-gate.ps1'),
            'Stop → pre-stop-commit-gate.ps1',
            'Aggiungere pre-stop-commit-gate.ps1 in Stop',
        ),
        ((): RuleResult => {
