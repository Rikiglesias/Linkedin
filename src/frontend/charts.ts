...
export function renderInvitesChart(trend: TrendRow[]): void {
    ...
    if (invitesChart) {
        ...
        (invitesChart.data.datasets[0] as { data: number[] }).data = invites;
        (invitesChart.data.datasets[1] as { data: number[] }).data = messages;
        (invitesChart.data.datasets[2] as { data: number[] }).data = acceptances;
...
export function renderRiskGauge(riskScore: number, healthScore: number): void {
    ...
    if (riskChart) {
        (riskChart.data.datasets[0] as { data: number[] }).data = [clampedRisk, 100 - clampedRisk];
        (riskChart.data.datasets[1] as { data: number[] }).data = [clampedHealth, 100 - clampedHealth];
