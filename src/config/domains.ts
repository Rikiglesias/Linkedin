...
export function buildRuntimeDomainConfig(accountProfiles: AccountProfileConfig[]) {
    return {
        ...
        integrationProxyPoolEnabled: parseBoolEnv('INTEGRATION_PROXY_POOL_ENABLED', true),
...
export function buildLimitsAndRiskDomainConfig() {
    return {
        ...
        complianceDynamicWeeklyWarmupDays: Math.max(1, parseIntEnv('COMPLIANCE_DYNAMIC_WEEKLY_WARMUP_DAYS', 180)),
...
export function buildAiDomainConfig() {
    return {
        ...
        inboxAutoReplyMinConfidence: Math.min(1, Math.max(0, parseFloatEnv('INBOX_AUTO_REPLY_MIN_CONFIDENCE', 0.9))),
...
export function buildCommsAndBusinessDomainConfig() {
    return {
        ...
        dailyReportAutoEnabled: parseBoolEnv('DAILY_REPORT_AUTO_ENABLED', true),
        dailyReportHour: Math.min(23, Math.max(0, parseIntEnv('DAILY_REPORT_HOUR', 20))),
        ...
        disasterRecoveryRestoreTestEnabled: parseBoolEnv('DR_RESTORE_TEST_ENABLED', true),
        disasterRecoveryRestoreTestIntervalDays: Math.max(1, parseIntEnv('DR_RESTORE_TEST_INTERVAL_DAYS', 7)),
        disasterRecoveryRestoreKeepArtifacts: parseBoolEnv('DR_RESTORE_TEST_KEEP_ARTIFACTS', false),
        securityAdvisorEnabled: parseBoolEnv('SECURITY_ADVISOR_ENABLED', true),
        securityAdvisorIntervalDays: Math.max(1, parseIntEnv('SECURITY_ADVISOR_INTERVAL_DAYS', 30)),
        securityAdvisorDocMaxAgeDays: Math.max(1, parseIntEnv('SECURITY_ADVISOR_DOC_MAX_AGE_DAYS', 45)),
        securityAdvisorAuditLookbackDays: Math.max(1, parseIntEnv('SECURITY_ADVISOR_AUDIT_LOOKBACK_DAYS', 30)),
        ...
        followUpDelayStddevDays: Math.max(0, parseFloatEnv('FOLLOW_UP_DELAY_STDDEV_DAYS', 1.2)),
...
export function buildProxyDomainConfig() {
    return {
        ...
        proxyQualityCheckEnabled: parseBoolEnv('PROXY_QUALITY_CHECK_ENABLED', true),
...
export function buildBehaviorDomainConfig() {
    return {
        ...
        rampUpNonLinearModelEnabled: parseBoolEnv('RAMPUP_NON_LINEAR_MODEL_ENABLED', true),
        rampUpModelWarmupDays: Math.max(7, parseIntEnv('RAMPUP_MODEL_WARMUP_DAYS', 180)),
        ...
        timingMinSlotSample: Math.max(1, parseIntEnv('TIMING_MIN_SLOT_SAMPLE', 6)),
        timingScoreThreshold: Math.min(1, Math.max(0, parseFloatEnv('TIMING_SCORE_THRESHOLD', 0.3))),
        timingRecentWindowDays: Math.max(1, parseIntEnv('TIMING_RECENT_WINDOW_DAYS', 30)),
        timingRecentWeight: Math.min(1, Math.max(0, parseFloatEnv('TIMING_RECENT_WEIGHT', 0.35))),
        timingBayesPriorWeight: Math.max(0, parseFloatEnv('TIMING_BAYES_PRIOR_WEIGHT', 8)),
        timingMaxDelayHours: Math.max(1, parseIntEnv('TIMING_MAX_DELAY_HOURS', 72)),
        timingAbLookbackDays: Math.max(1, parseIntEnv('TIMING_AB_LOOKBACK_DAYS', 30)),
        timingAbSignificanceAlpha: Math.min(0.25, Math.max(0.001, parseFloatEnv('TIMING_AB_SIGNIFICANCE_ALPHA', 0.05))),
