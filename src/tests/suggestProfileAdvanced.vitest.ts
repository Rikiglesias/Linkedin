...
describe('suggestConfigProfile — exhaustive boundaries', () => {
    ...
    it('1000 giorni, 10000 connessioni → aggressive', () =>
        expect(suggestConfigProfile(1000, 10000)).toBe('aggressive'));
    it('365 giorni, 500 connessioni → moderate (connessioni basse)', () =>
        expect(suggestConfigProfile(365, 500)).toBe('moderate'));
    it('30 giorni, 5000 connessioni → conservative (giorni bassi)', () =>
        expect(suggestConfigProfile(30, 5000)).toBe('conservative'));
});

describe('CONFIG_PROFILES — internal consistency', () => {
    it('conservative.caps.followUpMax <= aggressive.caps.followUpMax', () => {
        expect(CONFIG_PROFILES.conservative.caps.followUpMax).toBeLessThanOrEqual(
            CONFIG_PROFILES.aggressive.caps.followUpMax,
        );
    });

    it('conservative.timing.challengePauseMinutes >= aggressive.timing.challengePauseMinutes', () => {
        expect(CONFIG_PROFILES.conservative.timing.challengePauseMinutes).toBeGreaterThanOrEqual(
            CONFIG_PROFILES.aggressive.timing.challengePauseMinutes,
        );
    });

    it('moderate è tra conservative e aggressive per hardInviteCap', () => {
        expect(CONFIG_PROFILES.moderate.caps.hardInviteCap).toBeGreaterThanOrEqual(
            CONFIG_PROFILES.conservative.caps.hardInviteCap,
        );
        expect(CONFIG_PROFILES.moderate.caps.hardInviteCap).toBeLessThanOrEqual(
            CONFIG_PROFILES.aggressive.caps.hardInviteCap,
        );
    });

    it('moderate è tra conservative e aggressive per riskStopThreshold', () => {
        expect(CONFIG_PROFILES.moderate.risk.riskStopThreshold).toBeGreaterThanOrEqual(
            CONFIG_PROFILES.conservative.risk.riskStopThreshold,
        );
        expect(CONFIG_PROFILES.moderate.risk.riskStopThreshold).toBeLessThanOrEqual(
