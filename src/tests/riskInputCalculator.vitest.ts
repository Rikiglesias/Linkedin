...
describe('calculateRiskInputs — funzione pura', () => {
    test('tutti zero → tutti ratio zero', () => {
        const result = calculateRiskInputs({
            ...
            totalAttempts24h: 0,
            failedAttempts24h: 0,
            selectorFailuresToday: 0,
            challengeCountToday: 0,
            invitesSentToday: 0,
            hardInviteCap: 20,
        });
    ...
    test('hardInviteCap zero → velocityRatio zero (no division by zero)', () => {
        const result = calculateRiskInputs({
            ...
            totalAttempts24h: 20,
            failedAttempts24h: 3,
            selectorFailuresToday: 1,
            challengeCountToday: 1,
            invitesSentToday: 15,
            hardInviteCap: 0,
        });
        expect(result.inviteVelocityRatio).toBe(0);
