...
describe('Milestone 1000 — full system integration', () => {
    ...
    it('risk engine + trust score + growth model → budget coerente', () => {
        const risk = evaluateRisk({ errorRate: 0, selectorFailureRate: 0, pendingRatio: 0.2, challengeCount: 0, inviteVelocityRatio: 0.3 });
        const trust = calculateAccountTrustScore({ ssiScore: 80, ageDays: 365, acceptanceRatePct: 35, challengesLast7d: 0, pendingRatio: 0.2 });
    ...
    it('backpressure + ramp model → volume control coerente', () => {
        ...
        const ramp = computeNonLinearRampCap({
            currentCap: 15, hardMaxCap: 25, accountAgeDays: 90,
            warmupDays: 60, channel: 'invite', riskAction: 'NORMAL',
            riskScore: 20, pendingRatio: 0.3, errorRate: 0.05,
            healthScore: 80, baseDailyIncrease: 1,
        });
    ...
    it('compliance health + risk → decisione operativa', () => {
        const health = evaluateComplianceHealthScore({
            acceptanceRatePct: 35, engagementRatePct: 25, pendingRatio: 0.3,
            invitesSentToday: 10, messagesSentToday: 5,
            dailyInviteLimit: 25, dailyMessageLimit: 35,
            weeklyInvitesSent: 40, weeklyInviteLimit: 100,
            pendingWarnThreshold: 0.5,
        });
        const risk = evaluateRisk({ errorRate: 0.1, selectorFailureRate: 0.05, pendingRatio: 0.3, challengeCount: 0, inviteVelocityRatio: 0.2 });
    ...
    it('explainRisk → human-readable output', () => {
        const explanation = explainRisk({ errorRate: 0.2, selectorFailureRate: 0.1, pendingRatio: 0.3, challengeCount: 0, inviteVelocityRatio: 0.1 });
    ...
    it('tutto il sistema funziona senza errori con input realistici', () => {
        expect(() => {
            evaluateRisk({ errorRate: 0.15, selectorFailureRate: 0.08, pendingRatio: 0.35, challengeCount: 0, inviteVelocityRatio: 0.25 });
            calculateAccountTrustScore({ ssiScore: 65, ageDays: 200, acceptanceRatePct: 28, challengesLast7d: 0, pendingRatio: 0.35 });
            validateMessageContent('Ciao, mi piacerebbe connetterci per discutere di opportunità.', { duplicateCountLast24h: 1 });
