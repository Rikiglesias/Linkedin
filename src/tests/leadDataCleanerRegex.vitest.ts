...
const mocks = vi.hoisted(() => ({ requestOpenAIText: vi.fn(), logWarn: vi.fn() }));
vi.mock('../ai/openaiClient', () => ({ requestOpenAIText: mocks.requestOpenAIText }));
...
describe('cleanLeadDataWithAI fallback regex-safe', () => {
    ...
    test('fallback non crasha con nome che contiene metacaratteri regex', async () => {
        mocks.requestOpenAIText.mockRejectedValue(new Error('AI down')); // forza il ramo catch/fallback
