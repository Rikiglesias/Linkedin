...
> Tabella completa e allineata al canonico `MANAGED_ROUTER_HOOKS` (`~/.claude/scripts/model-router-config.mjs`), non più un sottoinsieme storico.
> Tipi: **blocking** (HARD, può fermare l'azione) · **advisory** (inietta contesto/checklist da leggere e dichiarare) · **condizionale** (presente solo in una modalità router) · **async** (log/audit post-azione, non blocca).
> Ordine colonne = ordine di registrazione nel canonico per evento.

| Evento                                   | Hook                              | Tipo                | Stato                         | Motivo                                                                                                                                                                                                  |
| ---------------------------------------- | --------------------------------- | ------------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SessionStart`                           | `ensure-claude-model-router.ps1`  | sync config         | condizionale (unified router) | Mantiene disponibile il router modello/provider; presente solo quando `ANTHROPIC_BASE_URL=http://127.0.0.1:4319`. In modalita' Anthropic nativo viene filtrato via da `applyManagedClaudeCodeSettings`. |
...
| `SessionStart`                           | `session-start-sync-check.ps1`    | sync advisory       | attivo                        | Verifica che canonici/memoria/todos siano allineati a inizio sessione e segnala drift.                                                                                                                  |
| `UserPromptSubmit`                       | `user-prompt-router-switch.ps1`   | sync config         | attivo                        | Applica lo switch router/modello richiesto via prompt (`/or:*`, `/anthropic`).                                                                                                                          |
| `UserPromptSubmit`                       | `ensure-claude-model-router.ps1`  | sync config         | condizionale (unified router) | Mantiene coerente provider/modello a ogni prompt; presente solo in unified router mode, filtrato in modalita' Anthropic nativo.                                                                          |
...
| `UserPromptSubmit`                       | `zero-rules-enforcer.ps1`         | sync advisory       | attivo                        | Re-inietta le zero-rules (DECIDE-DON'T-ASK e simili) prima del ragionamento.                                                                                                                            |
...
| `PreCompact auto`                        | `block-auto-compact.ps1`          | blocking            | attivo                        | Intercetta il compact automatico per forzare l'handoff prima del degrado.                                                                                                                               |
...
| `Stop`                                   | `stop-continuity.ps1` + `stop-completion-gate.ps1` | sync advisory       | attivi (stop-proactive-next-step ARCHIVIATO, assorbito) | Reiniettano obbligo di prossimo passo concreto, tracciamento todolist e completion report alla chiusura.                                                                                                |
| `Stop`                                   | `stop-completeness-check.ps1`     | sync advisory       | attivo                        | Verifica completezza 360° (zero-J) prima di chiudere il task.                                                                                                                                           |
| `Stop`                                   | `stop-auto-track.ps1`             | async audit         | attivo                        | Traccia automaticamente stato/findings di fine turno.                                                                                                                                                   |
| `Stop`                                   | `stop-todolist-goal-reminder.ps1` | sync advisory       | attivo                        | Ricorda di aggiornare la todolist viva e di suggerire `/goal` per end-state multi-turno.                                                                                                                |
| `Stop`                                   | `stop-decide-dont-ask.ps1`        | sync advisory       | attivo                        | Enforce DECIDE-DON'T-ASK: niente menu «vuoi X o Y?» su scelte tecniche o ordine dei task identificati dall'AI.                                                                                           |
| `Stop`                                   | `stop-selfcorrection-rule.ps1`    | sync advisory       | attivo                        | Quando l'utente corregge: forza root cause + rafforzo regola/hook nello stesso turno (zero-B.7).                                                                                                        |
| `Stop`                                   | `stop-sync-obsidian.ps1`          | async              | attivo                        | Sync unidirezionale memoria/todos/findings verso il vault Obsidian a ogni Stop.                                                                                                                         |
...
| `PostToolUse Bash`                       | `post-bash-package-change.ps1`    | async audit         | attivo                        | Rileva variazioni a package.json/lockfile e segnala re-install/audit dipendenze.                                                                                                                        |
...
| `PostToolUse Edit/Write/MultiEdit`       | `post-edit-auto-commit.ps1`       | async gated action  | attivo                        | Valuta auto-commit dopo edit verificato; rispetta gate e precondizioni (no `git add .`, no `--no-verify`).                                                                                              |
...
| `PostToolUse Edit/Write/MultiEdit`       | `post-edit-checks.ps1`            | sync advisory       | attivo                        | Check L2-L7 dichiarativo + valutazione pulizia codebase (hygiene) dopo edit codice. Fonde gli ex `post-edit-verify-checklist` + `post-edit-codebase-hygiene` (archiviati 2026-06).                       |
| `SubagentStop`                           | `subagent-stop.ps1`               | async log           | attivo                        | Traccia output subagent.                                                                                                                                                                                |
| `TeammateIdle`                           | `teammate-event.ps1 -Event Idle`  | async log           | attivo                        | Traccia agent teams (idle).                                                                                                                                                                             |
| `TaskCreated`                            | `teammate-event.ps1 -Event Created`| async log          | attivo                        | Traccia agent teams (task creato).                                                                                                                                                                      |
| `TaskCompleted`                          | `teammate-event.ps1 -Event Completed`| async log        | attivo                        | Traccia agent teams (task completato).                                                                                                                                                                  |

Totale operativo: il conteggio **non va hardcodato** in questo documento — va derivato dal canonico `MANAGED_ROUTER_HOOKS` in `~/.claude/scripts/model-router-config.mjs` (`applyManagedClaudeCodeSettings` lo materializza in `~/.claude/settings.json`). Allo stato attuale (2026-06-05) il canonico definisce **44 script hook unici** e **48 voci hook** in unified router mode (`ensure-claude-model-router.ps1` è registrato su due eventi — `SessionStart` + `UserPromptSubmit` — ed è il solo condizionale). In modalita' Anthropic nativo entrambe quelle voci vengono filtrate, quindi `settings.json` materializza **46 voci hook** distribuite sugli eventi Claude Code (verificato `node` su `settings.json`: SessionStart 4, UserPromptSubmit 9, PreToolUse 6, PreCompact 3, Stop 9, PostToolUse 11, SubagentStop 1, TeammateIdle 1, TaskCreated 1, TaskCompleted 1 = 46). Le 44 voci uniche scendono a 46/48 perché `inject-runtime-brief.ps1` (`UserPromptSubmit`+`PreCompact`) e `teammate-event.ps1` (`TeammateIdle`+`TaskCreated`+`TaskCompleted`) sono registrati su più eventi. La tabella sopra elenca tutte le voci canoniche (incluse le condizionali): è l'inventory completo, non più un sottoinsieme storico. Non aumentare gli hook senza miss ricorrenti misurati o senza un advisory ad alto valore che renda visibile un miss sistemico. Verifica del numero reale: `npm run audit:hooks` (allineato al canonico, non a questa tabella).
...
- "Dare sempre continuita'": va in runtime brief + checklist finale + reminder `skill-activation.ps1` + `Stop` hook `stop-completion-gate.ps1 (ex stop-proactive, archiviato)`; diventa blocking solo se emerge un pattern verificabile di false completion.
...
| L2-L6 advisory (caller, edges, scenari, UX, E2E)                                                     | `~/.claude/CLAUDE.md` L2-L6                            | `post-edit-checks.ps1` richiede dichiarazione applicabile       | sync advisory          | attivo                | No                           |
| L7 cross-domain per file                                                                             | `~/.claude/CLAUDE.md` L7                               | `post-edit-checks.ps1` advisory (parziale)                      | sync advisory          | attivo                | Parziale                     |
| L8 coerenza cross-file                                                                               | `~/.claude/CLAUDE.md` L8                               | nessuno specifico (delegato a `/verification-protocol`)                   | —                      | inattivo              | **Sì**                       |
| L9 truthful completion                                                                               | `~/.claude/CLAUDE.md` L9                               | `stop-completion-gate.ps1 (ex stop-proactive, archiviato)` advisory parziale                          | sync advisory          | attivo                | Parziale                     |
| Anti-ban LinkedIn (6 domande + 9 principi, `.claude/rules/browser-antiban.md`)                       | AGENTS.md `Priorita' assoluta` (domanda zero + pointer) | `pre-edit-antiban.ps1` blocking + `post-edit-antiban-audit.ps1`           | blocking + async       | attivo                | No                           |
...
| Codebase hygiene file diretto/indiretto                                                              | AGENTS.md `Cross-domain` + runtime brief               | `post-edit-checks.ps1` richiede dichiarazione                   | sync advisory          | attivo                | No                           |
...
| Cambio chat basato su contesto, token, crediti e costo                                                | runtime brief + preferences.md                         | `turn-governor-hook.ps1` (inietta `TOKEN_COST_CHAT_SWITCH`; in Codex il parity hook è `.codex/hooks/token-cost-context.ps1`) | sync advisory          | attivo                | No                           |
...
2. **Pazienza vs fretta** — promuovere a check del `stop-completion-gate.ps1 (ex stop-proactive, archiviato)`: se la risposta cita >5 affermazioni "fatto", richiedere dichiarazione esplicita "verificate al 100%: X/Y".
...
4. **Verifica 100%** — promuovere a sub-check del `post-edit-checks.ps1`: aggiungere domanda "ogni claim 'verificato' supportato da check reale eseguito in questo turno?".
