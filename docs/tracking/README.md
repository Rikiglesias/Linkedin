...
| Cambio `.mcp.json` project-scope | `.mcp.json`, `docs/tracking/AI_CAPABILITY_ROUTING.json` se cambia capability, `docs/tracking/AI_ADK_CAPABILITY_GOVERNANCE.json` se cambia layer/placement, `docs/tracking/ENGINEERING_WORKLOG.md` | `npm run audit:mcp-config` + prova `/mcp` in Claude Code |
...
| Cambio chat / continuità | `/lastchat save` (scrive `~/.claude/LASTCHAT.md`, sistema UNICO 2026-06-07), `C:/Users/albie/memory/`, `todos/active.md`, Obsidian `Resources/continuita/` (vista), `docs/tracking/ENGINEERING_WORKLOG.md` | `/lastchat` (read) — `CONTINUATION`/`SESSION_HANDOFF`/`SESSION_PROMPT` + `audit:handoff-staleness` ELIMINATI |
