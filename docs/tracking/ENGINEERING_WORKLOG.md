...
## 2026-09-05 — chat #29 — blocco 2 di F1: i gemelli del campione (scheduler, guardian, preflight, trust, checklist) e i consumer

**Tema**: goal `bot-operativo`, Fase 1, contratto frozen `3336bd29…` (digest ricontrollato prima degli edit al binding). Ripresa da `/lastchat` #28: HEAD `598b81a` = origin, tree pulito, `pre-modifiche` exit 0 (225 file / 2258 test).

**Blocco 2 = C3 + C4 + C5 + C21 (+ C6 chiuso)** — TDD rosso-prima: 6 file di test nuovi, 24 rossi osservati per feature mancante (15 verdi = casi «come oggi» da preservare), poi verde.
- **C4 scheduler**: `getLeadInvitedTotalsForLists` (`leadsCore.ts`, `COUNT(*) … invited_at IS NOT NULL GROUP BY list_name`) accanto a `getLeadStatusCountsForLists`; `evaluateAdaptiveBudgetContext(statusCounts, riskAction, { invitedTotal })` usa gli invitati REALI come denominatore e campione (1 INVITED + 1 BLOCKED-con-invited_at → 0.5, non 1.0) e applica il gate di C1: sotto 20 nessun `list_pending_*` (prima 1/1 = 1.0 → factor 0.25 → budget 0); fail-closed su campione incoerente/non finito. `ListScheduleBreakdown` espone `pendingInvitedTotal`, `pendingSampleSufficient`, `blockedSampleSufficient`.
- **C21 guardian**: `heuristics()` conta `pendingRatio ≥ 0.78` / `blockedRatio ≥ 0.35` solo con campione sufficiente (1 SKIPPED su lista senza inviti non è più `critical` → niente pausa 180 min al primo run). Soglie invariate.
- **C5 ③ riskAssessor**: `pendingFactor` via `pendingRatioSample` (1/1 → 0; 20/20 → 25). **⑥ trust score**: `getAccountTrustInputs` espone `invitedTotal`; `calculateAccountTrustScore` neutro sotto campione (stesso score/budget di un account senza inviti, accelerazione non bloccata), campione assente → fail-closed (penalità odierna). **⑦ antiBanChecklist**: pending ratio da `getRiskInputs` (denominatore = invitati, non tutti i 348 lead: 15/20 = 75% ora blocca, 15/363 = 4% lo nascondeva); literal `pendingCount > 10` sostituito dal gate.
- **C3 consumer**: `doctor.ts` PENDING_RATIO passa dal gate (`evaluateCompliance` esportata `@internal`; `dynamic.invitedTotal` + `pendingSampleSufficient`); `GET /api/kpis` +`riskInputs{pendingRatio,invitedTotal,pendingSampleSufficient}` accanto a `risk` (scelto: additivo; scartato allargare `RiskSnapshot` = 60+ letterali + file anti-ban); `/risk/explain` già gated da C1; daily report Telegram «Pending Ratio: X% (N invitati totali[, campione < 20: non pesa])» — testato end-to-end con `sendTelegramAlert` mockato e router Express reale via supertest.
- **VERIFY del contratto eseguite ESATTE**: C3 8/8 · C4 12/12 · C5 7 file 59/59 + `grep -rl "risk/sampleGate"` = 7 · C6 18/18 · C21 7/7. `/antiban-review` SICURO sull'intero blocco (nessun timing/volume/stealth toccato; sotto 20 invitati il ratio non frena, restano cap e challenge = STOP; la checklist diventa più severa sopra campione).
- Legacy adattati: `scheduler.vitest` (3° argomento), `aiGuardianAndIntent`/`unit` (campi nuovi, liste sopra campione), `workflowPreflightChecklist` (mock `getRiskInputs`), `workflowPreflightRiskAssessor` (fixture 10/10 → 20/20, stessa classe del blocco 1).
- **Gate `conta-problemi` exit 0 = 231 file / 2304 test** (da 225/2258), `tsc` 0 (backend + frontend), lint 0 warning.
- Residui DICHIARATI (`~/todos/improvements-proposed.md` 2026-09-05): `list_blocked_warn` (×0.6) non gated dal campione; `AccountTrustInputs.invitedTotal` opzionale fail-closed; guardia grep C5 soddisfatta anche via commento nei 2 file same-dir (`./sampleGate`) e nel guardian (consuma il flag di C4).

**Cosa resta di F1**: C7 (integrazione `runWorkflow` reale, browser mockato), C8-C11 (blocco 3), C12-C17 (blocco 4), C18-C20 (leve + sonde `scripts/probe/`). F1 = 7/21 `[pass]`. Binding `~/todos/bot-operativo.md` § Stato.

## 2026-09-05 — chat #29 (ripresa 10:45) — blocco 2-bis: la review indipendente trova il deadlock ancora vivo su Postgres

**Tema**: goal `bot-operativo`, F1. Il reviewer subagent della notte era morto per «session limit» senza verdetto → review rilanciata come Workflow ultracode `wf_f10acf6b-d5f` (6 lenti opus: correttezza, anti-ban, contratti, test/falso-verde, async, igiene + critico di completezza; ogni finding attaccato da 3 refutatori). 43 finding grezzi, 2 BLOCK veri, ~10 WARN, resto NIT.

- 🔴 **BLOCK ①, verificato alla fonte**: `src/db.ts` crea il `Pool` pg senza `setTypeParser` → su Postgres (DB di produzione: `db.ts:727` rifiuta SQLite) `COUNT(*)`/`SUM(int)` arrivano come STRINGHE (int8). Il repo lo sa altrove (`metrics.ts:65-68`, `incidents.ts:31`, `blacklist.ts` usano `Number(...)`) ma i lettori dei risk inputs no → `Number.isFinite('20')` = false → il gate del blocco 1 va in `invalid_sample_fail_closed` = «campione sufficiente» → **il deadlock del primo invito restava intatto in produzione** (i test girano su SQLite, che restituisce number: falso verde per costruzione). Fix radice: parser INT8 nel pool; fix al bordo: `Number()` in `getRiskInputs`, `getAccountTrustInputs`, `countLeadsByStatuses`, `getLeadStatusCountsForLists`, `getLeadInvitedTotalsForLists`, `riskAssessor`. Test `riskInputsPostgresCounts.vitest.ts`: DB mockato che restituisce stringhe → tutti number, 1/1 → NORMAL; sentinella sulla presenza del parser.
- 🔴 **BLOCK ②**: il campione del `blockedRatio` usava il proprio denominatore (inviti + esiti + skippati) con la soglia degli inviti: 20 SKIPPED senza un solo invito = «sufficiente» → guardian `critical` → pausa 180 min (stessa classe del deadlock; comportamento identico a prima, ma C21/U2 lo volevano chiuso). Ora campione = inviti REALI della lista, senza la regola «coppia incoerente» (skippati senza inviti sono normali). Deviazione dalla lettera di C21 dichiarata nel binding (U* > C*), EXPECT invariati.
- Gemelli chiusi: alert Telegram per-lista (`orchestrator.ts:174`) senza campione → `selectWorstPendingList` pura + test; wiring contesto→breakdown mai eseguito da un test → `applyAdaptiveContextToBreakdown` pura + test con mappa completa dei campi + sentinelle sui call-site (E2E a C7); `/api/v1/automation/snapshot` gemello di `/kpis` → `riskInputs`; `evaluateCooldownDecision` e ramo `watch` del guardian sul ratio grezzo → `RiskSnapshot.pendingSampleSufficient?` (assente = come oggi) + test; `rampUpWorker` non lineare sul pending grezzo → flag dello snapshot; ratio per-lista > 1 → clamp; prompt del guardian AI con nota sul campione; `/metrics` con campione; openapi; `antiBanChecklist` legge i risk inputs solo per outreach, try/catch fail-closed; report «1 invitato totale»; test più discriminanti (2 liste + 1 fuori filtro), robusti alla concorrenza, caso divergente di C4 fissato.
- Rinviati con causa: `~/todos/improvements-proposed.md` (2026-09-05, 6 voci). `guardian.ts` a 305 righe (>300) giustificato, split candidato.
- Gate e commit: vedi § Stato del binding e i commit `[1/2 codice]`/`[2/2 test]` di questo blocco.

## 2026-09-05 — chat #30 — blocco 3 di F1: il primo invito passa da `lead-approve`, la promozione cieca dei NEW è spenta

**Tema**: goal `bot-operativo`, F1, contratto frozen `3336bd29…` → C7 + C8 + C9 + C10 + C11 (F1 = 12/21 `[pass]`). Metodo: TDD rosso-prima per criterio, orchestrator/scheduler/repository REALI sul DB di test (copia), `/antiban-review` SICURO sull'intero blocco.

- **C8**: `AUTO_PROMOTE_NEW_LEADS_ENABLED` (default false, `CONFIG_REFERENCE.md` rigenerato): lo scheduler non promuove più fino a 100 NEW alla cieca; il dry-run resta in parità (conta i NEW solo con flag ON, tetto `min(listBudget, hardInviteCap*4)`). Unico default di comportamento cambiato in F1.
- **C9**: nuovo comando `lead-approve <id> [--reason <testo>] [--min-score <n>]` — eleggibilità UNICA in `core/leadInviteEligibility.ts` (5 filtri: lista vuota, lista non attiva, opt-out GDPR, campagna attiva, score sotto soglia; per-lead E per-insieme sulle stesse clausole SQL, `INVITE_NO_ACTIVE_CAMPAIGN_CLAUSE` riusata da `getLeadsByStatusForList`), approvazione in UNA transazione (verifica → `transitionLead` → ricontrollo, `core/leadApproval.ts`; rifiuto = rollback, lead NEW senza evento, exit 1 col nome del filtro e il comando per sanarlo). Il preflight di send-invites conta/anteprima con la stessa eleggibilità: prima prometteva READY_INVITE con GDPR o campagna attiva che lo scheduler non accoda.
- **C10**: a 0 READY_INVITE il preflight stampa `.\bot.ps1 lead-approve <id>` ESATTO (id scelto dalla stessa eleggibilità), anche nella «Prossima azione» del report non-TTY (`shared.ts`: nextAction del blocco preflight = warning critici); GUIDA § «4. Primo invito» (flag, `.env` del primo run senza `HARD_INVITE_CAP=1`, canary `--limit 1 --note none --no-enrich --dry-run`).
- **C7**: `firstInviteNoQuarantine.vitest.ts` attraversa `runWorkflow` REALE (entry guards, scheduler, riskEngine, guardian euristico, incidentManager veri; mockato solo il confine browser/job-runner/Telegram): 10 NEW + 1 INVITED → `completed`, 0 `quarantineAccount`, 0 `pauseAutomation`; controllo con `pendingRatioMinInvited = 1` → `blocked`. Orologio fermo in orario feriale (`tests/helpers/fakeWorkingHourClock.ts`: evita OUT_OF_HOURS, weekend, skip-day del 5%), throttle «fatto ora» per canary/heartbeat/backup/rescore, sink eventi NONE, site-check post-run OFF.
- **C11**: `citedCommandsExist.vitest.ts` (AST TypeScript: solo stringhe, i commenti non contano) → 2 comandi inesistenti corretti: `npm run cli` → `.\bot.ps1 unquarantine` (`loopCommand.ts`), `npm run proxy:doctor` → `bot.ps1 proxy-status` (`proxyLaunchPlan.ts`).
- Scoperta: la premessa del contratto «348 lead a score NULL» è superata (0 NULL sul DB reale, misurato) → lo score conta solo con `--min-score`, come nella query dei candidati dello scheduler (deviazione dichiarata nel binding § Stato).
- Verifica: `pre-modifiche` 234/2328 → `conta-problemi` exit 0 = **240 file / 2365 test**, `tsc` 0, `lint` 0, `madge --circular` 0 (568 file). Residui: `~/todos/improvements-proposed.md` (2026-09-05, blocco 3).
- Commit: serie `[1/3 codice]` `[2/3 test + GUIDA]` `[3/3 worklog]`; push dopo review indipendente (file anti-ban toccati: `scheduler.ts`, `sendInvitesService.ts`, `proxyLaunchPlan.ts`).
- **Review indipendente pre-push** (`typescript-reviewer`, 24 file riletti, 37/37 + 16/16 test rieseguiti): **BLOCK** su un byte NUL in `citedCommandsExist.vitest.ts:54` (`.join` con 0x00 al posto dello spazio: git trattava il test come binario, `git diff --stat` = «Bin») → sostituito, 0 byte NUL nel blob; WARN chiusi nello stesso commit: predicato «campagna attiva» in una sola costante `ACTIVE_CAMPAIGN_STATE_PREDICATE` (leadsCore.ts) usata da clausola NOT EXISTS e percorso per-lead, id di `lead-approve` = primo positional come gli altri comandi; WARN rinviati con causa (`~/todos/improvements-proposed.md`): side-effect di `transitionLead` annidato se il ricontrollo rigetta (live-event/feedback AI fantasma, stato persistito corretto), tetto dry-run per-lista vs promozione globale (flag OFF). Commit `4087f1a` [4/4 review]; gate completo rilanciato prima del push.

## 2026-09-05 — chat #31 — blocco 4a di F1: `doctor --no-browser`, stdout dei comandi JSON pulito, lock `workflow.runner` per account

**Tema**: goal `bot-operativo`, F1, contratto frozen `3336bd29…` → C15 + C16 + C17 (F1 = 15/21 `[pass]`). Ordine scelto: i tre criteri che NON toccano `src/browser|workers` prima del cluster anti-ban C12-C14 (banco 3 pass, contesto fresco per la review anti-ban). Metodo: TDD rosso-prima per criterio, DB di test reale per il lock, prova dal vivo sul binario costruito.

- **C15**: nuovo `cli/commands/doctorCommand.ts` — `--no-browser` → `runDoctor({ skipBrowserSessionCheck: true })` (lo stesso ramo del loop gate), senza flag check completo; help aggiornato. **Scoperta**: `node dist/index.js doctor --no-browser > out.json` NON era parsabile: dotenv 17 stampa 2 «tip» su stdout, `config/index.ts:18` logga il profilo attivo, il logger emette gli INFO con `console.log`, tutto prima che il comando parta. Il comando non può ripulire ciò che è già uscito → guardia `cli/jsonStdout.ts` installata in testa a `index.ts` PRIMA di config/dotenv: per i SOLI comandi il cui stdout è un JSON (`doctor`, `kpi`, `sync-status`, `incidents`, `status`, `diagnostics`/`diag`, `config-validate`) mette `DOTENV_CONFIG_QUIET`, devia `process.stdout.write` (quindi anche `console.log`) su stderr e riserva il vero stdout a `writeJsonResult`; `--help` escluso. Gli altri comandi non cambiano.
- **C16**: misurato sul binario costruito: `preflight-env` exit 0 («Ambiente OK», WARN Ollama spento e proxy assente su `default`), `config-validate` exit 0 (`errorCount 0`). Nessun BLOCKED-lever.
- **C17**: `core/workflowRunnerLock.ts` — chiave SEMPRE `workflow.runner:<accountId>` (`default` incluso), risolta in `loopCommand.ts` DOPO l'eventuale `--account` (prima: `workflow.runner` senza flag vs `workflow.runner:<override>` → due runner sullo stesso account `default` potevano convivere); `system.ts` `listRuntimeLocksByPrefix` (`LIKE 'workflow.runner%'`, anche la chiave nuda di un daemon pre-C17); `health.ts` daemon check e `adminCommands.ts` (`runnerLock` = il più recente + `runnerLocks`) osservano tutti i namespace. Test sul DB reale di test: takeover del lock stale (`acquire_stale_takeover`, il vecchio owner non rinnova più), runner concorrente rifiutato (`acquire_contended`), caso misto senza flag + `--account <stesso>` → secondo `acquired:false`, sentinelle statiche sui 3 siti.
- Test: `doctorNoBrowserFlag` 5/5 · `jsonStdoutGuard` 8/8 (vitest intercetta il `console` globale: il test usa una `Console` legata ai veri stream; la prova del `console` reale è il binario) · `runtimeLockStaleTakeover` 3/3 · `runtimeLockConcurrentRunner` 6/6.
- Verifica: `pre-modifiche` 240/2365 → `conta-problemi` exit 0 = **244 file / 2387 test**, `tsc` 0, `lint` 0, `madge --circular` 0; dal vivo: `doctor --no-browser` → stdout 5284 byte che iniziano con `{`, parse exit 0, `tasklist` camoufox/firefox 0 prima / 0 dopo, `doctor --help` su stdout, `status` JSON parsabile. `/antiban-review` SICURO (nessun browser/timing/stealth; meno sessioni, meno runner concorrenti).
- Residui dichiarati (`~/todos/improvements-proposed.md`, blocco 4a): dotenv `quiet: true` globale; gemelli JSON non nell'insieme (`restore-drill`, `security-advisor`, `privacy-cleanup`); lock del leader multi-account; chiave legacy `workflow.runner` di un daemon pre-C17 (fermarlo prima del deploy); `preflight-env` e `config-validate` in disaccordo sul proxy.
- Commit: serie `[1/3 codice]` `[2/3 test]` `[3/3 worklog]`; push dopo la review indipendente del blocco 4 (insieme a C12-C14).

## 2026-09-05 — chat #31 — blocco 4b di F1: guardia UA↔engine fail-closed, prova di invio ancorata, nessun secondo invito al retry

**Tema**: goal `bot-operativo`, F1, contratto frozen `3336bd29…` → C12 + C13 + C14 (F1 = 18/21 `[pass]`; restano C18-C20 = leve + sonde). Metodo: `/antiban-review` PRIMA di ogni edit (SICURO su `stealth.ts`, `ja3Validator.ts`, `inviteWorker.ts`, `inviteStateProbe.ts`), TDD rosso-prima, harness su Chromium vero per la parte DOM.

- **C12** `browser/stealth.ts`: `useJa3Proxy` non bypassa più la guardia UA↔engine (CycleTLS spoofa il JA3 delle richieste proxate, ma Gecko vs Blink resta osservabile dalla pagina: uno UA Chrome su Camoufox è spoofing); pool locale senza UA coerente → errore che nomina engine e famiglia attesa (mai il pool intero); cloud tutto incoerente → fingerprint LOCALE + warn; cloud misto → solo i coerenti. `proxy/ja3Validator.ts`: con engine camoufox/firefox + proxy lo status è `SECURE` e la recommendation dice di lasciare `USE_JA3_PROXY=false` (JA3 nativo NSS coerente con lo UA Firefox del pool); ramo chromium resta `GAP` senza la frase vietata (grep del contratto = 0 in tutto `src`, test inclusi: la frase è composta a runtime nel test). Il test esistente `dispositivoStabileNelTempo` mockava `detectBrowserFamily → 'chromium'` (famiglia inesistente) e passava solo grazie al fallback tolto: corretto a `'chrome'` + `browserEngine`.
- **C13** nuovo `browser/inviteStateProbe.ts` + chiavi `profileActionsContainer`/`systemNoticeContainer` in `selectors.ts`: prova di invio = Pending nelle azioni del profilo TARGET o toast «Invitation sent» di sistema; limite settimanale = solo in modale/toast/alert. `inviteWorker.ts` senza `page.textContent('body')` (grep 0/0). Harness `tests/harnessInviteProofAnchored.ts` su Chromium vero: 8/8, con 2 controprove che mostrano il vecchio criterio (pagina intera) rispondere true su un Pending in sidebar e su un post del feed.
- **C14** `inviteWorker.ts`: subito dopo il gate `isProfileDeleted` (pagina già sul profilo, nessuna chiamata AI sprecata) e PRIMA del cap che incrementa `invites_sent`, la sonda pre-click: già Pending → `transitionLead(INVITED, 'invite_already_pending')`, 0 click, `invites_sent` invariato. Test `inviteProofRetryNoDuplicate` (4): caso positivo, ordine sonda→cap provato con `invocationCallOrder`, dry-run senza sonda, sentinelle statiche.
- Verifica: `conta-problemi` exit 0 = **247 file / 2403 test** (da 244/2387), `tsc` 0, `lint` 0; VERIFY esatte del contratto: C12 16/16 + grep 0/1, C13 grep 0/0 + harness exit 0, C14 4/4; caller rilanciati (`actionWorkers`, `selectorsAndDomains`, `unit`) verdi.
- Residui dichiarati (`~/todos/improvements-proposed.md`, blocco 4b): un solo UA Firefox mobile nel pool; `pickDeterministicFingerprint` con fallback al pool desktop (gemello irraggiungibile); JA3 di Chromium headless non misurato; canary senza pagina-profilo per i nuovi contenitori; 3 gemelli fuori F1 (`audit.ts`, `acceptanceWorker.ts`, `observePageContext.ts`); `invites_sent` sotto-conta di 1 al retry «già pending» (lettera del contratto) e le stat per-lista/bandit non vedono quell'invito.
- Commit: serie `[1/3 codice]` `[2/3 test]` `[3/3 worklog]`; auto-push sospeso (file anti-ban) → **review indipendente pre-push = primo passo della chat successiva** (checkpoint per tetto contesto 49%).

## 2026-09-05 — chat #32 — F1: review pre-push del blocco 4b (PUSH_OK), sonde C18/C19/C20 versionate, C20 verde, script pulizia C19 provato su copia

**Tema**: goal `bot-operativo`, F1, contratto frozen `3336bd29…` (digest ricomputato dopo gli edit = invariato). F1 = **19/21 `[pass]`**: restano C18/C19, il cui «DOPO» è la leva (scrittura sul DB vero). Metodo: reviewer indipendente `typescript-reviewer` in background sul diff `dd5ef4a..9264277` mentre scrivevo le sonde (file disgiunti); `/antiban-review` SICURO sul solo fix al probe; sonde provate PRIMA sul positivo noto (zero-M «la sonda può mentire»).

- **Review indipendente 4b**: `VERDETTO: PUSH_OK`, 0 BLOCK, C12/C13/C14 VERO con file:riga; vitest 4/4 file + suite intera 247/247, `tsc`/lint exit 0, harness 8/8. 3 WARN: ① `inviteStateProbe.ts` inghiottiva gli errori Playwright in silenzio → **chiuso** (`11bb01e`: log `invite_probe.dom_error` con il passo fallito, valore neutro invariato, log fire-and-forget; sentinella `inviteProbeDomErrorLogged.vitest.ts` 4 casi); ② fallback `main section:has(h1)` ampio e ③ nessuna guardia gemella C14 in `messageWorker.ts` → rinviati con causa (`improvements-proposed.md`, blocco «4b review»), insieme all'opzione tri-state scartata (un `unknown` dopo il click perderebbe l'incremento del cap).
- **C18** `scripts/probe/incidents-open.cjs` (read-only, `OPEN_READONLY`): `open=[1,2,3,4,5,6]`, `maxId` 6, nessun id nuovo → PRIMA verificato; comandi della leva in binding § Leve.
- **C19** `scripts/probe/dbq.cjs`: `test` 2980 · `telegram.polling_error` 2968 · `selector_fallbacks` 3/3 · `dynamic_selectors` 2/2 · `run_logs.total` 31413. Script `scripts/maintenance/clean-test-residue.cjs` (dry-run di default, `--yes` per cancellare, backup `VACUUM INTO` verificato con COUNT, `BEGIN IMMEDIATE`, `--level2` opzionale, solo SQLite, `leads` mai toccata): **provato END-TO-END su una COPIA del DB** → `deleted=2980,2968,3,2;level2=19687`, `dbq` dopo 0/0/0/0, totale 31413→5778 (torna esatto); sul DB vero SOLO dry-run.
- **C20** `scripts/probe/probe-deps.cjs` (legge `dist/config`+`dist/accountManager`, stampa solo booleani/status, errori di rete ridotti al codice): `accounts[0].id='default'`, `telegram.ok=true`, `supabase.status=200`, `db.dialect='sqlite'`, camoufox `135.0.1 beta.24`, Ollama `ECONNREFUSED` = riga «GIÙ» della tabella → VERIFY = EXPECT → `[pass]`.
- Verifica: `conta-problemi` exit 0 = **247 file / 2403 test** (+ sentinella nuova 4/4 fuori conteggio), harness Chromium 8/8, prettier verde sui 5 `.cjs` (ESLint li ignora per config). Push verificato con `ls-remote` = `11bb01e`, ahead 0.
- Residui dichiarati: WARN ② e ③ (sopra); `.codex/agents/` untracked (decisione utente); sonda `probe-deps` non copre il proxy (non nell'EXPECT).
- Commit: `6addeba` sonde + script `[1/2]` · `11bb01e` fix probe + sentinella `[2/2]` · serie 4b `820ad82`/`53fd86f`/`9264277` — **tutto PUSHATO**. Prossimo: leve C18/C19 (`ok C18` / `ok C19`) → grade F1 firmato → `/verification-protocol` → renegotiate v2 per F4/F5.

## 2026-09-06 — chat #33 — design del contratto v2 (F4/F5) riconciliato con Codex cieco; 6 difetti nuovi confermati alla fonte; F1 resta 19/21 (leve C18/C19)

**Tema**: goal `bot-operativo`. Ripresa verificata alla fonte (ahead/behind 0/0 su `636e53b`; sonde C18 `open=[1,2,3,4,5,6]` e C19 `2980/2968/3/2` invariate; sessione LinkedIn `lastVerifiedAt` 2026-06-09). Mentre C18/C19 restano leve utente, lavoro INDIPENDENTE: design della v2 (F4 osservabilità + F5 versione migliore) come blocco di DESIGN (GATE-COSTRUZIONE-360 scalato: ricerca = report 360 + `npm view`/`npm outdated` + codice riletto; ≥3 alternative; failure-modes per categoria; review cross-model = Codex cieco; accettazione in parole utente). Nessun edit di codice: il contratto v1 è frozen (digest `3336bd29…` ricomputato dopo ogni edit al binding = invariato).

- **Bozza v2** `~/todos/bot-operativo-v2-bozza.md` (fuori dal binding: il digest v2 hasha ogni riga `- C<n>`): perimetro P1-P41 classificato (a) prima del login/primo invito · (b) post-E2E · (c) mai · (d) fuori goal; criteri C22-C51 in 5 blocchi (A identità/runtime · B azione sicura · C esiti/alert · D prova su mondo sintetico + lifecycle · E post-E2E); ogni ancora `[LETTO]` riletta oggi (`launcher.ts:482-504`, `utils.js:345-349,381`, `humanClick.ts:20-21`, `human/inputBlock.ts:228-231`, `index.ts:2-8,112,158`, `jobRunner.ts:107,683`, `broadcaster.ts:141`, `doctor.ts:248-266`, `selectorCanary.ts:86,192-201`, `uiFallback.ts:366`, `observePageContext.ts:132`, `workflowEntryGuards.ts:112-122`, `auth.ts:71-119`, `telegramListener.ts:161-185`, `incidents.ts:26-28` + `001_core.sql:93` + valori DB).
- **Scoperta che cambia l'ordine**: l'identità browser persistita (oggi Camoufox lanciato senza `os`/`fingerprint` → device casuale a ogni lancio) è prerequisito del **LOGIN** (F2), non «dopo il primo E2E»: il dispositivo del login deve essere quello dell'invito → `bot.ps1 login` va lanciato DOPO il blocco A, su sessionDir nuova. Codex cieco converge in modo indipendente.
- **Codex cieco pre-piano** (`codex exec -s read-only`, 437 s, 32 KB; primo lancio bloccato sullo stdin del background → ucciso e rilanciato con `</dev/null`): 3 divergenze ACCOLTE/ADATTATE (camoufox 0.12 = dopo, con trigger misurato; identità mancante/mismatch = BLOCCO non rigenerazione; run_id = prima) + **6 difetti NUOVI confermati file:riga**: doppia compensazione del cap (`inviteWorker.ts:720` −1, `throw` → catch `:780` −1; `stats.ts:695` senza clamp) · **budget 1 → 0** (`applyAdaptiveFactor` `scheduler.ts:223-232` ritorna 0 con factor <1; `list_blocked_warn` ×0,6 `:358-361` non gated dal campione; warn 0,25 `domains.ts:167`) = deadlock-gemello di C4 al primo run `--limit 1` con 1 SKIPPED · AI azione vuota = PROCEED (`aiDecisionEngine.ts:322`) + delay extra da `confidence` in 3 worker · `--list` riattiva liste inattive (`scheduler.ts:690`, `leadsCore.ts:1140-1148` vs `leadInviteEligibility.ts:166`) · lock rilasciato PRIMA di chiudere il browser (`index.ts:112`) · destinatario per slug parziale (`navigationContext.ts:172`). 1 respinta (naming test), 3 parziali (§ 8 della bozza).
- **Errore mio (ledger)**: il residuo v1 «`list_blocked_warn` conservativo, mai 0, nessun deadlock» (chat #29) era un'inferenza non letta → `~/memory/errors/2026-09-06-residuo-mai-0-non-verificato.md`; tracker `improvements-proposed.md:4506` corretto con la riga barrata.
- Micro-leva chiusa: `.codex/agents/` (4 toml specchio di `.claude/agents`, nessun segreto) versionati in `395d22f`, auto-push verificato (`636e53b..395d22f`). Residuo: manca lo specchio di `antiban-review`.
- Verifica: nessun codice toccato → `conta-problemi` non rilanciato (ultimo verde 247/2403 posteriore all'ultimo edit di codice, chat #32); digest contratto invariato (verificato 2×); tree pulito.
- Prossimo (leve): frase di Riccardo che autorizza `renegotiate` v2 + «ok C18»/«ok C19» → R1 evaluator vergine sulla bozza → ratify/freeze v2 → blocco A (C22-C28) → **poi** login. Senza le leve la condition del goal non può diventare vera → `/goal clear`, ripresa `chat nuova → /lastchat → /goal bot-operativo`.


## 2026-09-06 — chat #34 — «autorizzo» → contratto v2 riaperto (C22-C51 nel binding), R1 a 4 lenti + refutatori; «ok tutto» → C18/C19 eseguiti sul DB vero, F1 = 21/21

**Tema**: goal `bot-operativo`. Ripresa via `/lastchat` verificata alla fonte (HEAD `ef8a8af` = origin 0/0; contratto v1 frozen `3336bd29…`; sonde C18 `open=[1,2,3,4,5,6]` e C19 `2980/2968/3/2` invariate; nessun `codex` vivo). Riccardo ha scritto «autorizzo» (leva ① rinegoziazione) e poi «ok tutto» (leve C18/C19 + via libera a ratificare).

- **`renegotiate` FIRMATO** (`set-contract-state.ps1 -Action renegotiate -UserQuote "autorizzo" -Note "F4/F5: C22-C51…"`, dry-run verde prima): evento ledger 17:08:45, `frozen → negotiating`, backup `bot-operativo.md.pre-renegotiate.bak`. Il contratto d'uso del tool esige status frozen + hash coerente + evento freeze FIRMATO + quote verbatim + clausole nominate: tutti soddisfatti.
- **Contratto v2 nel binding**: C22-C51 (30 criteri, blocchi A identità/runtime · B azione sicura sul path invito · C esiti veri/alert · D prova sintetica + lifecycle · E post-E2E) in forma ferrea `- C<n> [todo]`, CF4, nota v2, log negoziazione, `contract_version: 2`. Modello letto dal tool: 55 criteri; 473 VERIFY estratti, 0 non sicuri (`Test-VerifyCommandSafe`). Clausola accolta da Codex: nessun PASS offline dimostra un invito riuscito né assenza di rischio ban.
- **R1 = Workflow `wf_7060577f-3d4`** (input ESATTO `r1_input.md`: end_state + U1-U4 verbatim + CF + prime righe C1-C21 + proposta, MAI il ragionamento del builder; vietata la lettura di todos/lastchat/worklog/memory): 4 evaluator VERGINI a lente (scope · test-deboli · edge · anti-ban; opus, effort xhigh) → merge/dedup → 2 refutatori FATTUALI per obiezione (verificano file:riga, non il giudizio). Esito nel blocco successivo.
- **Fatti pre-raccolti per la riconciliazione** (letti alla fonte): nessun npm script `harness:*`/`test:e2e:synthetic`; nessun sottocomando `camoufox-fetch`/`identity-init`/`preflight-identity`/`runs`; harness esistenti in `src/tests/harness{DomContracts,InputBlockEvents,InviteProofAnchored}.ts` (non `src/scripts/`); `package.json` senza `engines` né override `playwright-core`; installati playwright 1.58.2 + camoufox-js 0.9.3; binario Camoufox `135.0.1-beta.24` (`%LOCALAPPDATA%\camoufox\camoufox\Cache\version.json`); crash → `performGracefulShutdown` → `exit(0)` (`index.ts:2-8,158`); doppia compensazione cap (`inviteWorker.ts:720` + catch `:780`); `applyAdaptiveFactor` → 0 con factor <1 (`scheduler.ts:223-232`); setup vitest copia il DB reale via `VACUUM INTO` e imposta solo `DB_PATH` (`globalSetup.ts:34-40`, `vitestSetup.ts:14`), `DATABASE_URL` non neutralizzata.
- **C18 ESEGUITO** («ok tutto»; un precedente «ok C18» arrivato sul canale SUBAGENT_RESULT era un'eco e NON è stato eseguito): `.\bot.ps1 db-backup` → `data/backups/backup-2026-09-06T15-44-43-167Z.sqlite` (7.176.192 byte, SHA256 `42c14d5…`); `incident-resolve 1…6` → 6× «Incidente N risolto»; sonda `incidents-open.cjs` DOPO: `open=[]`, i sei `RESOLVED` con `resolved_at` 2026-09-06, `newOpenBeyondContract=[]` → **C18 `[pass]`**.
- **C19 ESEGUITO**: dry-run = candidati `{test:2980, polling:2968, selector_fallbacks:3, dynamic_selectors:2}`; `clean-test-residue.cjs --yes` → backup proprio `data/linkedin_bot_backup_clean-2026-09-06T15-48-46.sqlite` (31420 righe dentro), `run_logs.total` 31420 → 25472 (−5948 = 2980+2968 esatto), `deleted=2980,2968,3,2`; sonda `dbq.cjs` DOPO 0/0/0/0 → **C19 `[pass]`**. Il totale 25472 (non 25465 previsto nelle leve) = +7 righe `plugin_loader.ready` scritte dai 7 comandi di C18; `leads` non toccata; `--level2` non eseguito (non richiesto, resta disponibile).
- **F1 = 21/21 `[pass]`**; grade formale al freeze v2 (il contratto è `negotiating`). Tracker: binding § Stato ×3, § Leve, checkbox F1.T1/F1.T2; register `active.md:18`.
- Verifica: nessun codice toccato → `conta-problemi` non rilanciato (ultimo verde 247/2403, chat #32); tree pulito; backup DB ignorati da git (`.gitignore:5,7`).
- Prossimo: esito R1 → riconciliazione alla fonte → R2-PROPOSTA nel binding → `ratify` (quote «ok tutto») → `freeze` v2 + `grade` → blocco A (C22-C28) con TDD rosso-prima + `/antiban-review`; `bot.ps1 login` SOLO dopo il blocco A.

## 2026-09-06 — chat #35 — contratto v2 R3 ratificato/congelato/gradato (hash `a781f9ae…`, PARTIAL 21/61): refutatori R1 + R2-CRITICA 30/30 accolte alla fonte

**Tema**: goal `bot-operativo`. Ripresa via `/lastchat` verificata alla fonte (HEAD `e706b36` = origin 0/0, tree pulito; i 2 Workflow lasciati in background dalla chat #34 risultano COMPLETATI, output su disco: `w4tvrp94b.output` 872 KB, `wwtaslq5e.output` 47 KB).

- **Refutatori R1 sui 29 BLOCK** (estratti con script compatto — id, `factual_status` ×2, `correction` troncata — mai il dump intero, do-not-redo della #34): tally 1 CONFIRMED · 25 PARTIAL · 3 contesi (scope:O5, antiban:R1-AB-06, R1-AB-11) · 0 REFUTED netti. 9 correzioni di TESTO applicate alla R2: URL hardcoded sul path invito = `navigationContext.ts:145/207/235` (428/443/562/568 fuori path; nessun ramo naviga su `lead.linkedin_url`); `countRecentIncidents` = finestra troncata alla mezzanotte UTC (`' ' < 'T'`), NON «sempre 0» → rosso-prima riscritto (incident nella stessa data UTC del cutoff); retry in-run «già acquisito» via `visitedProfilesToday` (`jobRunner.ts:404`, `inviteWorker.ts:331-343`) → il caso rosso è il run successivo; `--limit` mancante viene SOSTITUITO da `hardInviteCap` (`sendInvitesService.ts:212`), non «nessun limite»; pausa 429 default 180 (`domains.ts:68`, il `?? 60` è morto) + conflitto a 3 vie feed-probe/checkLogin/voyager; cookie rilevati per layout Gecko E Chromium; 3 punti neutri della sonda (`inviteStateProbe.ts:44/49/64`) + `hasInviteSentNotice`; riferimenti `package.json:109/136-140`.
- **R2-CRITICA (2 lenti, 34+63 claim)**: 30 obiezioni (15 BLOCK · 15 MAJOR), **30/30 accolte** dopo rilettura MIA alla fonte di 18 fatti (tutti confermati): `missclick.ts` HA un chiamante (`humanBehavior.ts:16,185-186`, la R2 lo dava per dead code → 4ª voce allowlist C30); `humanClick.ts:88` (`clickLocatorHumanLike → clickCoordinatesHumanLike` → sentinella C55 su grafo con arco escluso); `touchGestures.ts:29,66` tap grezzi; `REAPPLY_INTERVAL_MS=1_000` (`windowInputBlock.ts:22`) → cap C31 = 10 derivato, non 6; gemello `countDistinctIncidentAccounts` (`incidents.ts:44-47`) in C59; DUE copie inline della traduzione migration (`db.ts:589-592` e `:671-672`) → `normalizeMigrationForPg` in C61; `GUIDA.md:211/213` vs `domains.ts:98/20` (C60 rosso nominato); `scheduler.ts:223-232/339-361` → C35 0→1 solo con `CONTINUE` e senza `global_risk_*`/`list_pending_*`; `pool.ts:72-166` UA 132-135 → C23 major UA = major binario; `stealth.ts:33-42` → coerenza UA/TLS sul file a ogni lancio; `workflowEntryGuards.ts:125-147` body-wide INVARIATA (C32); `stats.ts:777/797` + `leadsCore.ts:1240-1252` → `OUTBOUND_UNKNOWN` pending presunto con `invited_at` (C34/C57); `inviteWorker.ts:614/764-778` → asserto rosso di C57 nominato; `index.ts:2-8/100/158` → C36 su binario reale + modulo unico handler; C64 → artefatto versionato `docs/antiban/verdicts.json` con `blob_sha` + perimetro esteso (`src/fingerprint/**`, `scheduler.ts`, `workflowEntryGuards.ts`, `src/config/**`, `aiDecisionEngine.ts`); C43 → run su ENTRAMBI gli engine + copia read-only del DB reale + `dbq.cjs` ESTESA con delta per tabella; timing (C63/C31/C41) → sequenze byte-identiche a seed fisso, ZERO bande; C26/C36 → nessuna env di test nel binario (regola ⑥); C28/C45 → isolamento in processo figlio, asserto rosso = `pgConnectAttempts===0`.
- **Inventario C22-C64 (R2-T14)**: 40 attivi (C22-C32, C34-C46, C48-C53, C55-C64), **C33 ESPUNTO** (R1 AB-09: pagina LinkedIn in più nel canary; capacità coperta da C32+C13) e **C47** → `~/todos/improvements-proposed.md`; **C54 mai assegnato**. Decisioni dichiarate nel § Log negoziazione (zero-C.10, con scartato-perché).
- **Testo R3** scritto in `scratchpad/contract_block_v2_r3.md` e innestato nel binding (backup `bot-operativo.md.pre-r3.bak`); log negoziazione + R2-CRITICA v2 + R3-PROPOSTA v2.
- **Ledger firmato** (`set-contract-state.ps1`, dry-run verde prima): `ratify` 23:19:31 (quote «ok tutto», autorizzazione data da Riccardo PRIMA del testo R2/R3, dichiarato nella nota) → `freeze` 23:19:32 → `grade` 23:20:03 = **PARTIAL 21/61** (C1-C21 PASS, C22-C64 FAIL = `[todo]`, CF1-4 non gradati). Modello: 65 criteri, **836 VERIFY tutti safe**, hash `a781f9ae2bfe…`; `GRADING.md` generato dagli stati del binding (parser sulle righe `- C<n> [stato]`, 61/61 righe) in `~/.claude/scripts/contratto-bot-operativo/`. § Stato del binding aggiornato DOPO il freeze: digest ricomputato invariato (gli stati fuori dai `- C<n>` non entrano nell'hash).
- Verifica: nessun codice `src/` toccato → `conta-problemi` non rilanciato (ultimo verde 248/2407, chat #34); tree pulito prima del worklog.
- Prossimo: **blocco A** (C28 → C22 → C23 → C52 → C24 → C25 → C26 → C27 → C53): `npm run pre-modifiche`, TDD rosso-prima, `/antiban-review` sui file del perimetro C64, sonda `antiban-coverage.cjs` + `docs/antiban/verdicts.json` DENTRO il blocco (C64 vale al suo stop), review indipendente pre-push; poi B1 con C63 per primo. `bot.ps1 identity-init --new-session` + `login` SOLO dopo il blocco A.

## 2026-09-06 — chat #35 — blocco A avviato: C28 chiuso (harness isolati con runtime condiviso, isolamento della suite in processo figlio, dotenv idempotente)

**Tema**: goal `bot-operativo`, contratto v2 R3 frozen `a781f9ae…`. Metodo: `superpowers:test-driven-development` (ogni criterio rosso-prima), Plan Mode scartato (il contratto È il piano), edit inline.

- **ROSSI osservati prima del codice**: `testIsolation.vitest.ts` (figlio vitest con `DATABASE_URL=postgres://fittizio` → `new Pool` in `db.ts:716`, exit 1); `harnessRuntime.vitest.ts` (modulo assente); `dotenvIdempotente.vitest.ts` (`loadDotEnv` ricaricava il `.env` a ogni chiamata, quindi una chiave cancellata tornava).
- **`src/tests/harnessRuntime.ts` (NUOVO, ~200 righe)**: isolamento come effetto dell'import (`loadDotEnv()` → `delete DATABASE_URL/SUPABASE_URL` → `SESSION_DIR`/`DB_PATH` in `os.tmpdir()`), server HTTP locale per le fixture (`serve(html)`, `echoHeadersUrl()`), `context.route('**/*')` che nega e conta ogni host ≠ `127.0.0.1/localhost/[::1]`, `requestfinished` esterno = leak, control-case `fetch('https://www.linkedin.com/')` con `AbortSignal.timeout` (lint L3.3) che deve risultare bloccato, riga `HARNESS_RESULT` JSON, exit 0/1/2 (2 = sonda rotta: 0 richieste locali o 0 pagine). Funzioni pure (`isLocalRequestUrl`, `buildHarnessSummary`, `harnessExitCode`) coperte da 7 test.
- **3 harness esistenti cablati** (`harnessDomContracts`, `harnessInputBlockEvents`, `harnessInviteProofAnchored`): runtime importato per primo, fixture via `page.goto(serve(html))` al posto di `setContent` (così le richieste locali si contano), nessun `chromium.launch` diretto. **`harnessIdentity.ts` (NUOVO)**: Camoufox VERO senza `user_data_dir` su pagina locale, misura `navigator.userAgent === header User-Agent` (Firefox/135 = binario 135.0.1-beta.24) — base per C23/C24.
- **Script npm**: `harness:dom`, `harness:input-block`, `harness:invite-proof`, `harness:identity`, `harness:all` (catena `&&`). **`vitestSetup.ts`**: `loadDotEnv()` poi `delete` delle due chiavi remote; **`env.ts`**: guardia `dotEnvLoaded` (idempotente per processo: dotenv non sovrascrive ma RI-AGGIUNGE le chiavi assenti; unico chiamante di produzione `config/index.ts:16`).
- **Difetto di strumento** (do-not-redo): l'heredoc del tool Bash ha trasformato la sequenza backslash-n in newline reale dentro le stringhe TS patchate via Python → stringa spezzata (`TS2304 Cannot find name 'CONTRATTI'`); riparato con `chr(92)+'n'` nel sorgente Python. Le patch su file con righe vuote contenenti spazi vanno fatte per regex, non per blocco letterale.
- **VERIFY C28**: `npm run harness:all` **exit 0** — dom `localhost_requests=3/pages_loaded=3`, input-block 2/2, invite-proof 9/9, identity 2/3; per tutti `external_requests=0`, `blocked_attempts=0`, `control.blocked_attempts=1`, `session_dir`/`db_path` sotto `%TEMP%` e ≠ `data/`; `testIsolation.vitest.ts` figlio exit 0 (`pgConnectAttempts===0`, `DATABASE_URL`/`SUPABASE_URL` undefined nel worker, `dbPath` in tmpdir); `dotenvIdempotente` 1/1; eslint 0 warning; `tsc --noEmit` 0.
- 🔴 **Regressione introdotta dalla guardia e trovata dal gate completo** (`post-modifiche` **exit 1**: 251/252 file, 2415/2418 test): `configEnvTwoFileLoading.vitest.ts` 3/4 rossi — chiama `loadDotEnv()` più volte con cwd diversi, ma la guardia era già scattata in `vitestSetup`. Fix: quel test importa il modulo FRESCO per caso (`vi.resetModules` + import dinamico), stessa tecnica di `dotenvIdempotente`; scartato un `reset` esportato (codice solo-test in un modulo di produzione). Errore mio a ledger: ho letto «exit 0» del comando composto (l'`echo EXIT` finale) invece dell'`EXIT 1` nel log → il commit `28be899` è partito con la suite rossa; corretto nel commit successivo.
- Binding: `C28 [pass]` (digest invariato `a781f9ae…`, dry-run grade), § Stato 23:5x; `GRADING.md` C28 → PASS (22/61). Residuo dichiarato: `env.ts` ∈ perimetro esteso C64 (`src/config/**`) → verdetto `/antiban-review` in `docs/antiban/verdicts.json` al gate di fine blocco A (sonda `antiban-coverage.cjs` + file ancora da costruire dentro il blocco). Auto-push SALTATO dal hook (area anti-ban: `harnessInputBlockEvents.ts`) → il push attende la review indipendente di fine blocco A, come da contratto.
- Prossimo (chat nuova, contesto al 41%): C22 (pin esatto `playwright`/`playwright-core`/`camoufox-js`, `engines`, `camoufoxBinaryVersion`, sonda `engine-pin.cjs`, sottocomando `camoufox-fetch --check`) → C23 → C52 → C24 → C25 → C26 → C27 → C53 → C64 sul diff del blocco → review indipendente pre-push → push.

## 2026-09-07 — chat #36 — blocco A: C22 chiuso (pin esatto della combinazione collaudata, guardia fail-closed prima di Camoufox(), `camoufox-fetch --check`, sonda `engine-pin.cjs`)

**Tema**: goal `bot-operativo`, contratto v2 R3 frozen `a781f9ae…` (digest invariato dopo `C22 [pass]`, grade dry-run PARTIAL 23/38). Metodo: `superpowers:test-driven-development` (rosso-prima), Plan Mode scartato (C22 congelato = piano, Codex cieco già in #33), `/antiban-review` **SICURO** su `camoufoxRuntime.ts` + `launcher.ts`.

- **Scoperta a monte che ha guidato il design**: camoufox-js 0.9.3 risolve la cache a OGNI lancio con `camoufoxPath(downloadIfMissing = true)` (`pkgman.js:276-295`, raggiunta da `launchPath()` e da `getPath()` per fontconfig/properties.json): cache vuota o versione fuori dai vincoli della libreria → `install()` fire-and-forget dell'ULTIMA release GitHub mentre il lancio prosegue; stesso canale per gli addon di default (uBO, `addons.js:54-72`). Passare `executable_path` NON basta (`getPath` passa comunque da `camoufoxPath`). `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD` scartato: salta il download ma anche il caricamento di uBO (fail-open silenzioso, cambio di comportamento). Scelto: guardia solo-filesystem che RIFIUTA, download del tag esatto in un comando manuale.
- **ROSSI osservati prima del codice**: sonda `engine-pin.cjs` exit 1 con 7/11 check rossi (caret ×2, `engines` assente, override assente, `confPin` assente, `autoFetch` = guardia assente); `enginePin.vitest.ts` senza modulo; `citedCommandsExist` (C11) rosso su `bot.ps1 camoufox-fetch` citato in GUIDA.
- **`src/browser/camoufoxRuntime.ts` (NUOVO, 144 righe)**: `inspectCamoufoxRuntime`/`assertCamoufoxRuntimePinned` (`version.json` = pin + `addons/UBO`; `CamoufoxRuntimeError` code `CAMOUFOX_RUNTIME_NOT_PINNED` col comando che risolve), `pinnedCamoufoxAssetUrl` (tag esatto `v135.0.1-beta.24`, asset verificato con HEAD 200), `installPinnedCamoufox` (download in memoria → rm cache → `unzip` → `version.json` → addon → MMDB, riusando i primitivi ESM di camoufox-js: un guasto di rete non distrugge l'installazione). **`launcher.ts`**: import + 1 chiamata nel ramo `useCamoufox` prima di `await Camoufox({`. **`cli/commands/camoufoxFetch.ts` (NUOVO)**: `camoufox-fetch [--check]`, stdout solo JSON (`JSON_STDOUT_COMMANDS`), voce in `commandHelp.ts`, `case` in `index.ts`.
- **Config**: `camoufoxBinaryVersion` (`types.ts`, `domains.ts` ← `CAMOUFOX_BINARY_VERSION`), regola ERROR in `validation.ts` quando engine=camoufox e pin assente/malformato; chiave nel conf locale (gitignored) + `.conf.example` + `CONFIG_REFERENCE.md`. **`package.json`**: `playwright` 1.58.2 e `camoufox-js` 0.9.3 senza caret, `overrides["playwright-core"]="1.58.2"`, `engines.node ">=22"`; lock riconciliato con `npm install --package-lock-only` (+5/−2 righe, playwright-core entry unica 1.58.2, nessuna dipendenza aggiunta).
- **`scripts/probe/engine-pin.cjs` (NUOVO)**: 11 check (pin ×4, node, lock, `npm ls --all`, confPin, binary, `autoFetch` misurato eseguendo la guardia COMPILATA su una cache vuota con `fetch` stubbato → deve rifiutare con 0 chiamate + guardia prima di `Camoufox(` in `dist/browser/launcher.js`, identità `.fingerprint.json` se esiste); override `ENGINE_PIN_ROOT`/`_CAMOUFOX_CACHE`/`_SESSION_DIR` solo per i negativi. Prerequisito `npm run build` (come `probe-deps.cjs`).
- **VERIFY C22**: `node scripts/probe/engine-pin.cjs` **exit 0**, 11/11 ok (`identityUaMajor` saltato: file assente, lo crea C23); `node dist/index.js camoufox-fetch --check` **exit 0**, JSON `installed=pinned=135.0.1-beta.24`, `addonsMissing=[]`, nessun download; `config-validate` 0 errori col conf reale e **exit 1 con l'errore C22** a chiave vuota (regola anti-ban 10: entrambi i rami visti dal vivo, profilo attivo su engine camoufox → la guardia sta sul path reale); `enginePin.vitest.ts` 12/12 incl. i due negativi del binding (lock `9.9.9` → exit 1 `lockPlaywrightCore` rosso; UA `Firefox/133` su binario 135 → exit 1 `identityUaMajor` rosso; cache vuota → rifiuto con `fetch` mai chiamata); C11 verde; eslint 0 warning (10 `no-non-null-assertion` nel test rimossi); `tsc` 0; gate `post-modifiche` **exit 0 = 253 file / 2430 test** (da 252/2418).
- **do-not-redo (3ª occorrenza, stessa classe)**: nel heredoc del tool Bash una doppia barra rovesciata arriva a Python come singola → la sequenza barra+b diventa BACKSPACE (GUIDA e conf con `.<BS>ot.ps1`, riparati con `chr(92)`); dopo ogni patch che contiene barre rovesciate contare i byte 0x08 nel file (= 0). Testi con apostrofi rompono anche il heredoc stesso → script Python su file, mai inline; nuovi file → Write tool.
- **Residui dichiarati**: `installPinnedCamoufox` non esercitato in rete (URL del tag verificato con HEAD 200; `--check` è la VERIFY del contratto; mai lanciarlo con un browser Camoufox aperto); `npm audit` 33 vulnerabilità PREESISTENTI (lock cambiato di 5 righe) → igiene deps fuori C22; uBO su LinkedIn = scelta preesistente (adblock rilevabile) → da valutare in C60/C64; `launcher.ts` 962 righe (>300, preesistente) → split-plan fuori C22.
- Commit: `9d33f50` (src, 9 file) + `269a903` (deps/config/docs, 5 file) + questo (sonda + test + worklog). Gate git: commit per AREA (4 aree in uno = bloccato) → 3 commit. Auto-push SALTATO dal hook (area anti-ban) → push dopo la review indipendente di fine blocco A, come da contratto; ahead 5.
- Prossimo: C23 (identità persistita `.fingerprint.json` = UNICA sorgente, coerente con binario e host; `isUaTlsCoherentWithEngine` a ogni lancio) → C52 → C24 → C25 → C26 → C27 → C53 → C64 → review → push.

## 2026-09-07 — chat #37 — blocco A: C23 chiuso (identità persistita `.fingerprint.json` = UNICA sorgente, coerente col binario e con l'host) + 2 flaky della suite

- **Ripresa**: HEAD `c530df0`, tree pulito, ahead 5. Gate `pre-modifiche` 1° giro: notifica «exit 0» ma log EXIT=1 (exit del `tail` composto, letto il log): 2 flaky preesistenti → `riskInputsSampleFailClosed` C2(d) (race sulla copia DB unica per run: ora misura prima/dopo e pretende `getRiskInputs` nell'intervallo) e `workerRegistryAdvanced` (eseguiva i worker REALI con contesto `{}` → log a teardown = unhandled error; ora verifica `AsyncFunction` senza eseguire). Commit `a083226`, gate 253/2430 exit 0.
- **C23 (TDD, `/antiban-review` SICURO)**: rosso-prima `fingerprintPersistence.vitest.ts` (Cannot find module). Costruito: `src/browser/browserIdentity.ts` (regola pura: schema, `BrowserIdentityError` con `code`, `uaMajor`, `rewriteVersionStrings` = stessa regex di camoufox-js `_castToProperties`, `profileHasCookies` su ENTRAMBI i layout, lettura/validazione, scrittura atomica temp+rename mai overwrite, `ensureBrowserIdentity` = crea SOLO su profilo vergine), `browserIdentityRuntime.ts` (cablaggio: `engineBuild` = `version.json` Camoufox o `playwright-core/browsers.json`; Camoufox → browserforge `generateFingerprint` con `os` host + finestra reale, riscritto al major del binario; chromium/firefox → pool deterministico con UA riscritta al major Playwright; `identityWindow` estratta dal launcher), `browserIdentityProjection.ts` (le 3 proiezioni pure verso stealth script/contextOptions, `BrowserFingerprint`, opzioni Camoufox `fingerprint`+`os`+`config['fonts:spacing_seed']`), `cli/commands/identityInit.ts` (`identity-init [--new-session] [--account] [--os]`, JSON, `--os`≠host exit 1, profilo con cookie exit 1, idempotente), `stealth.ts` esporta `isUaTlsCoherentWithEngine`, `seedRuntime.ts` esclude il file identità dal «profilo già visto» (literal + test anti-drift, niente import da `browser/`), `launcher.ts` legge SOLO dal file (identità risolta UNA volta fuori dal retry, prima di ogni launch; pool/cloud mai al lancio; `-23` righe), help/jsonStdout/index, GUIDA § Requisiti.
- **Ordine dei check** (dal rosso del test): famiglia UA↔engine PRIMA del major (una UA Chrome su Gecko è «incoerente», non «major sbagliato»).
- **VERIFY**: `fingerprintPersistence.vitest` 13/13 (10 casi del binding + scrittura atomica + ordine guardia→launch + anti-drift); `semeFingerprintWiring` adattato (il seme arriva al pool via `ensureLaunchIdentity({accountId: seme})` → `generatePoolIdentity(params.accountId)`); C11 e C22 verdi; tsc 0, eslint 0, madge 0 cicli; gate `post-modifiche` **254 file / 2443 test exit 0**; `npm run build:backend` exit 0; **dal vivo**: `identity-init --os linux` exit 1 senza scritture, creazione exit 0 (`engineBuild 135.0.1-beta.24`, UA Firefox/135, os windows), idempotente `created:false`, profilo con `cookies.sqlite` exit 1; sonda `engine-pin.cjs` 11/11 con `identityUaMajor` ok; **harness `harness:identity`** su Camoufox VERO dal path di produzione: 3 lanci identici (navigator/screen/finestra/metriche font), UA JS = header = file, hardware/screen/colorDepth = file, `languages[0]` = locale, control-case con altro `fontsSpacingSeed` → metriche font DIVERSE (il seme del file è applicato), file byte-identico dopo il 1° e 3° lancio, egress 0 → `measures_failed 0, exit_code 0`.
- **Scoperte**: con `fingerprint` custom camoufox-js ignora `window` (solo `generateFingerprint`) e la finestra riportata viene dai dati browserforge (1920x1165 e 2752x1237 su due identità) → misura corretta a «platform coerente + finestra/screen stabili»; vincolo screen ≤ monitor tracciato per C24 (`improvements-proposed.md`). `LeakWarning custom_fingerprint` a ogni lancio = solo log (scelto NON `i_know_what_im_doing`). `deviceMemory` null su Firefox → C24 toglie l'iniezione.
- **Residui dichiarati**: cleanup del runtime harness `rmSync` EBUSY sul DB aperto dai lanci di produzione → try/catch + warn (stesso criterio di `globalSetup.ts`); `identity-init` sugli engine da pool usa solo il pool locale; `launcher.ts` 939 righe (split-plan già tracciato).
- Commit: src (13 file) + docs (GUIDA + questo worklog); push SOSPESO (area anti-ban: review indipendente di fine blocco A). Prossimo: C52 → C24 → C25 → C26 → C27 → C53 → C64 → review → push.

## 2026-09-07 — chat #38 — blocco A: C52 chiuso (engine e account bloccati al file identità, `preflight-env` fail-closed, preset starter → camoufox) + 5 fix dal critico indipendente di fine-C23

**Tema**: goal `bot-operativo`, contratto v2 R3 frozen `a781f9ae…`. Metodo: `superpowers:test-driven-development` (rosso-prima: `Cannot find module preflightEngineLock`), `completeness-critic` read-only sullo snapshot di C23 (10 finding riletti alla fonte prima di accoglierli), `/antiban-review` **SICURO**; Workflow/Plan Mode scartati (codice vivo cross-file → inline; criterio congelato = piano).

- **Ripresa**: HEAD `a225c3e`, tree pulito, ahead 8; `pre-modifiche` 254/2443 exit 0 (letto dal LOG). Suite hook `run-all.ps1` rilanciata: 7 FAIL tutti control-plane preesistenti, nessuno da C23/C52 → elenco in `~/todos/improvements-proposed.md`.
- **Critico di fine-C23 (10 finding)**: ACCOLTI ora 1 (`browserforge:null` su Camoufox → camoufox-js ne genera uno NUOVO a ogni lancio, `utils.js:345`: ora `IDENTITY_CORRUPT` in lettura E creazione), 2 (`existsSync`+`renameSync` = check-then-act che sovrascrive nella corsa fra `identity-init` e `login`: ora temp + `fs.linkSync` esclusivo, EEXIST → `IDENTITY_ALREADY_EXISTS`, temp sempre rimosso), 3 (`accountId` persistito ma mai validato, e il launcher passava il SEME mentre `identity-init` passava `profile.id`: ora `accountId` = profilo risolto con `profiloIdDellaSessione` + `IDENTITY_ACCOUNT_MISMATCH`, il seme è SOLO `poolSeed`), 6 (test «pool mutato» tautologico → test di FORMA: gli argomenti di `buildStealthInitScript` in `launcher.ts` vengono solo da `stealthInputs`), 8 (`identity-init --account <sconosciuto>` ricadeva sul primo profilo con exit 0 → `IDENTITY_ACCOUNT_UNKNOWN`, exit 1). RESPINTO 4 (tz/geo da IP a ogni lancio = coerenza geo↔proxy voluta; residuo: misurare `timeZone` nell'harness). → C24: 5 (mock `window` con lo screen browserforge in headless, `outerHeight+85` su Gecko), 7 (`deviceMemory ?? 8`). → test-infra: 9, 10.
- **C52 — `src/browser/browserIdentity.ts`** (294 righe): codici `IDENTITY_ENGINE_MISMATCH` (PRIMA del check binario: un cambio engine cambia anche il binario, l'errore giusto è «engine») e `IDENTITY_ACCOUNT_MISMATCH`; decisione dichiarata: engine ≠ file blocca ANCHE senza cookie (superset del criterio, fail-closed). Helper di versione (`uaMajor`, `engineBuildMajor`, `rewriteVersionStrings`) estratti in **`browserIdentityVersion.ts`** (NUOVO) e ri-esportati (consumer invariati: runtime, test, sonda). **`browserIdentityRuntime.ts`**: `LaunchIdentityParams` = `accountId?` + `poolSeed`. **`launcher.ts`**: `accountId: options.accountId, poolSeed: semeFingerprint`; log `browser.fingerprint_selected` con `accountId` vero + `poolSeed`. **`seedRuntime.ts`**: `profiloIdDellaSessione` esportata (nessun import da `browser/`: guardia `semeFingerprintWiring` verde). **`preflightEngineLock.ts`** (NUOVO, puro): cookie + `BROWSER_ENGINE` assente/`chromium` → FAIL con la riga esatta da mettere in `config/bot-settings.conf`; identità ≠ engine risolto → FAIL (rispecchia il launcher); profilo autenticato senza file → FAIL con `identity-init --new-session`; cablato in **`preflightEnv.ts`** (check 5b, 286 righe). **`identityInit.ts`**: `resolveIdentityInitTarget` esportata, `IdentityInitTargetError`, `poolSeed = accountId`. **`presets/starter.env.example`**: `BROWSER_ENGINE=camoufox` + nota `camoufox-fetch`/`identity-init`; gli altri 3 preset erano già camoufox.
- **Test**: `engineLockPerProfile.vitest.ts` (NUOVO, 17): 4 casi del criterio (Gecko/Chromium, vergine, senza cookie), 8 del preflight (assente, chromium, senza file, ok, vergine/mismatch, corrotto, cablaggio, preset), 5 dei fix (browserforge null, corsa con `existsSync` spiato, account, `--account` sconosciuto, forma stealth). `semeFingerprintWiring` adattato (`poolSeed`). `harnessIdentity.ts`: il control-case dichiara l'account (il 1° giro era rosso PER la nuova guardia: file copiato in cartella non configurata = `IDENTITY_ACCOUNT_MISMATCH`, voluto).
- **VERIFY**: 17/17 + C23 13/13 + wiring 11/11 + C22 verde; tsc 0, eslint 0 (max-warnings 0), madge 0 cicli; gate `post-modifiche` **255 file / 2460 test exit 0**; `build:backend` exit 0; dal vivo su `dist`: `identity-init --account x` exit 1 `IDENTITY_ACCOUNT_UNKNOWN`, `--os linux` exit 1, idempotenza (exit 0 `created:true` → exit 0 `created:false`, 0 tmp, `accountId=default`, `engine=camoufox`), `preflight-env` cookie + engine vuoto → **exit 1** con «BROWSER_ENGINE=camoufox», positivo ✅, mismatch ❌; `engine-pin.cjs` ok; **`npm run harness:identity` exit 0, measures_failed 0, egress 0** su Camoufox vero.
- **do-not-redo**: exit di una sonda letto dopo un `echo` intermedio = exit dell'echo (2ª occorrenza a ledger) → `cmd > file; echo EXIT=$?` sempre adiacente. Gate plan-mode W3 sul 2° file di codice: basta dichiarare il binding a fasi in 1 riga. Catena flag→edit nello stesso messaggio: 8/8 ok, MAI due flag per messaggio.
- **Residui dichiarati**: finding 5/7 → C24; 9/10 → test-infra; tz nell'harness → C24/C26; suite hook 7 FAIL control-plane (owner altri goal); `launcher.ts` ancora >900 righe (split-plan tracciato).
- Commit: src+test (1 area) · preset+docs (2 aree); push SOSPESO (area anti-ban → review indipendente di fine blocco A, poi push). Prossimo: **C24** (una sola identità per pagina: sezioni native Camoufox, via `deviceMemory`/font mock/`measureText`, screen ≤ monitor, viewport headless) → C25 → C26 → C27 → C53 → C64 → review → push → grade.

## 2026-09-07 — chat #39 — blocco A: C24 chiuso (una sola identità per pagina su Camoufox: registro `CAMOUFOX_NATIVE_SECTIONS`, guardie su 11 sezioni, screen browserforge coerente con l'host)

**Tema**: goal `bot-operativo`, contratto v2 R3 frozen `a781f9ae…`. Metodo: `superpowers:test-driven-development` (rosso-prima: `camoufoxNativeSections.vitest.ts` 13 FAIL, export mancante), `/antiban-review` **SICURO** (6 domande: meno marker, timing/volumi/ordine intoccati, nessuna identità di produzione esistente: `find .fingerprint.json` = 0), WebSearch triangolata (Camoufox docs + crawlex + scrapewise: l'iniezione JS è rilevabile via descrittori/`toString()`; Camoufox patcha in C++ proprio per non averla), sonde dal vivo (harness su Camoufox vero, geometria dell'host, inner/outer). Workflow e Plan Mode scartati: codice vivo cross-file → inline; criterio congelato.

- **Perimetro (zero-E, gemelli)**: il contratto chiedeva 3 sezioni (`fonts`, `devicememory`, `perfmemory`); il principio «nulla di nativo definito via JS» copre anche `window` (finding 5 del critico), `colordepth`, `platform`, `languages`, `language`, `webdriver`, `permissions` — fonte primaria il mapping `camoufox-js/dist/mappings/browserforge.config.js` (navigator.*, screen.*, window.inner/outer*). Superset dichiarato in binding. Regola 10 (ramo-che-decide): default `BROWSER_ENGINE=chromium` → set VUOTO → tutte le sezioni girano come prima; `camoufox` (preset starter) → set pieno; ENTRAMBI i rami provati dal vitest su pagina simulata.
- **`src/browser/stealthScripts.ts`** (781 righe, era 726; >300 preesistente): `CAMOUFOX_NATIVE_SECTIONS` esportato (spostato dal launcher, 18 chiavi); guardie `_skip.has()` sulle sezioni 2 webdriver, 4 languages, 6+10 permissions, 7 window, 12 deviceMemory, 13 colorDepth, 12-bis performance.memory, 14+H22 fonts, 15 platform, 16 language; `!_isFirefox` su deviceMemory/performance.memory/navigator.connection (API che Firefox non ha: marker sotto UA Firefox anche su Playwright Firefox, non solo Camoufox).
- **`src/browser/launcher.ts`** (933 righe, −9): importa il set da `stealthScripts`; `deviceMemory: stealthInputs.deviceMemory ?? undefined` (null nel file = niente iniezione); mock finestra headless (solo engine non-Camoufox) = viewport REALE del context (`viewport?.width ?? file`).
- **`src/browser/browserIdentityRuntime.ts`** (295 righe): `hostScreenGeometry` (WorkingArea + Bounds in UNA chiamata PowerShell; headless 1920x1080) con `identityWindow` wrapper; planner `generateBrowserforgeWithinHost` a 4 tentativi (① finestra ≤ screen ≤ monitor · ② screen ≤ monitor con finestra RIDOTTA allo screen, pavimento soft 90%×85% · ③ qualunque screen ≤ monitor · ④ screen ≥ finestra con avviso) con `strict: true` e invariante DURA `assertScreenContainsWindow` (screen ⊇ finestra: `handleWindowSize` centra la finestra nello screen, più piccolo = screenX/Y negativi visibili alla pagina). **Sonda sull'host REALE**: geometria 1463x866 / 1463x914 (laptop a 125%) → il primo design fail-closed «finestra ≤ screen ≤ monitor» BLOCCAVA la creazione («The current constraints are too restrictive») → planner → tentativo ② 1366x768 (o 1376x774), finestra ridotta; Camoufox vero headless con quel fingerprint: inner 1366x707, outer = screen = avail 1366x768, screenXY 95,0 (`innerWidth 0` nei dati generati è ignorato dal binario, che deriva inner da outer).
- **Harness** (`src/tests/harnessIdentity.ts` 249 + `harnessIdentityControls.ts` 76 + `harnessIdentitySnapshot.ts` 124, split NUOVO): misure C24 sul profilo di produzione (zero proprietà proprie su navigator/screen, deviceMemory/performance.memory undefined, fonts.check/measureText/permissions.query/getter webdriver/innerWidth `[native code]`, larghezze ripetibili, tz stabile, languages[0] = Accept-Language, finestra ≤ screen ≤ monitor, webdriver false/Notification default/prompt NATIVI) + control-case os=linux (Camoufox diretto in tmp + LO STESSO script del launcher). **Premessa del contratto smentita dal vivo**: `document.fonts.check('12px "Segoe UI"')` è `true` anche su linux (spec FontFaceSet: true per ogni famiglia senza FontFace da caricare; misurato `check('FamigliaInesistenteC24') = true`) → VERIFY letterale sostituito dalla misura per larghezza («Segoe UI, monospace» vs «monospace»: linux 318.5 = 318.5 → font nascosto; windows 319.5 ≠ 381.3 → disponibile), più forte; deviazione documentata in GRADING.md.
- **VERIFY**: `camoufoxNativeSections.vitest` 20/20 (set, guardie nel sorgente, launcher importa il set, rami Camoufox/Chromium/Firefox, planner 5 casi incl. geometria reale); correlati 8 file / 122 test verdi (`engineLockPerProfile` fix 6 adattato: viewport dal context); tsc 0, eslint 0, madge 0; **`harness:identity` su Camoufox VERO 18/18, measures_failed 0, egress 0, exit 0** (1° giro 16/17 = premessa fonts.check); gate `post-modifiche` **256 file / 2480 test exit 0**.
- **do-not-redo**: backtick in un COMMENTO dentro il template literal dello script = parse error (tsc lo prende subito); un vincolo su un dataset esterno va SONDATO sui dati reali dell'host PRIMA di renderlo fail-closed; una premessa sulla semantica di un'API (fonts.check) va letta dalla spec prima di finire in un VERIFY.
- **Residui dichiarati**: chromium headless: viewport forzato 1920x1080 ≠ `viewport` del file (coerente in pagina; engine non di produzione); harness: WARN `browser.fingerprint_seed_read_failed` (`sync_state` assente nello sqlite vuoto dell'harness, preesistente da C23); tz/proxy «stesso paese» → C26; `LeakWarning custom_fingerprint` resta fail-loud (deciso: nessun flag globale). Tracker: `~/todos/improvements-proposed.md`.
- Commit: src+test (1 area) · worklog (docs); push SOSPESO (area anti-ban → review indipendente di fine blocco A, poi push). Prossimo: **C25** (rumore seedato sull'account, `noiseGenerator.ts`) → C26 → C27 → C53 → C64 → review → push → grade.

## 2026-09-07 — chat #40 — blocco A: C25 chiuso (rumore canvas/webgl/audio seedato sull'account) + verdetti anti-ban versionati

**Tema**: goal `bot-operativo`, contratto v2 R3 frozen `a781f9ae…`. Metodo: `superpowers:test-driven-development` (rosso-prima 3 FAIL su 4), `/antiban-review` **SICURO** (6 domande), WebSearch triangolata su 3 fonti indipendenti, lettura della catena reale prima di toccare. Workflow e Plan Mode scartati: due file, criterio congelato, blast radius mappato con un grep.

- **Il difetto, letto alla fonte**: `pickDeterministicFingerprint` (`pool.ts:289`) sceglie la entry del pool a partire dall'`accountId`, ma `generateConsistentProfile` (`noiseGenerator.ts:41`) seminava con `${userAgent}|${id}` della **entry**. Il pool desktop ha ~20 elementi: due account che vi collidono esponevano canvas, webgl e audio **identici** ⇒ correlatore cross-account, la stessa classe già chiusa sul dwell dei tasti (`launcher.ts:288`, commento in loco).
- **Il test esistente mascherava il buco**: `canvasNoiseFloor.vitest.ts:37` chiamava `generateConsistentProfile({ ...base, id: 'account-N' })`, cioè simulava account diversi iniettando l'`id` nella entry — forma che passa anche col bug, mentre il launcher passa il fingerprint vero col `.id` del pool. Stesso pattern già visto in `audit-codebase`: verde nei test, monco in produzione. Corretto: l'account è un argomento, non un `id` finto.
- **`src/fingerprint/noiseGenerator.ts`**: `generateConsistentProfile(base, accountId)` con parametro **obbligatorio** — un campo opzionale sarebbe ricaduto in silenzio sul vecchio seme (fail-open), mentre così è il compilatore a impedire di dimenticarlo; guardia esplicita su `accountId` vuoto/whitespace (throw); seme `${userAgent}|${id}|${accountId}`.
- **`src/browser/launcher.ts`**: passa `identity.accountId` — sempre valorizzato e stabile per profilo dopo C23/C52 (`browserIdentityRuntime.ts:289` con fallback al seme congelato, `IDENTITY_ACCOUNT_MISMATCH` a guardia).
- **WebSearch (zero-C.8-bis)**: Castle (lato difensore/fraud detection), crawlex (lato scraping), GoUndetected (lato antidetect) convergono su un punto **oggettivo**: il rumore deve essere deterministico per profilo e diverso fra profili; quello per-chiamata o per-sessione è la via rapida al flag (due render identici con hash diversi = manipolazione esposta). Il design di C25 è esattamente questo.
- **Premessa mia smentita in corsa** (ledger `~/memory/errors/2026-09-07-negativo-dedotto-da-fatto-adiacente.md`): avevo scritto due volte «nessun profilo ha storia con LinkedIn», deducendolo da «zero inviti completati» e confermandolo con una sonda `find -name cookies.json` che su un profilo **Firefox** non può trovare nulla (usa `cookies.sqlite`). Realtà: `data/session` ha **24 cookie su linkedin.com/linkedin-ei.com** e `.session-meta.json` con `lastVerifiedBy: "login"` del 2026-06-09 — la memoria che citavo lo diceva già («`li_at` valido fino al 2027-06-13»). La decisione regge per altra ragione (quel profilo è già fail-closed senza `.fingerprint.json` per C23, e il piano prevede `identity-init --new-session`), ma il **vincolo** è ora scritto in un artefatto: `data/session` non va riusato.
- **`docs/antiban/verdicts.json` (NUOVO)**: anticipata la parte di C64 che non richiede la sonda — formato contrattuale `{file, blob_sha, answers[6], verdict, commit, date}`, 2 voci (`noiseGenerator.ts`, `launcher.ts`) col `blob_sha` reale di `f8bbb30`. I file di C22/C23/C24/C52 **non** sono stati inseriti: i loro verdetti sono scaduti (quei file sono cambiati dopo la review) e ricostruirli a memoria violerebbe zero-M — si ri-recensiscono sul blob attuale quando C64 costruirà `scripts/probe/antiban-coverage.cjs`.
- **VERIFY**: `noiseSeedAccount.vitest` (NUOVO) rosso-prima **3 FAIL / 4**, poi 4/4; correlati `canvasNoiseFloor` + `fingerprint-coherence` verdi (3 file / 44 test); tsc exit 0; gate `post-modifiche` **257 file / 2484 test exit 0**; commit `f8bbb30`, secrets-scan 0 su 5 file.
- **do-not-redo**: un test che costruisce l'input «come se» il fix ci fosse già non è una rete, è una conferma; quando una decisione poggia su un'ASSENZA, la sonda va provata su un positivo noto prima di crederle.
- Commit: src+test (1 area, `f8bbb30`) · docs (verdicts + worklog); push SOSPESO (area anti-ban → review indipendente di fine blocco A). Prossimo: **C26** (proxy fail-closed + `preflight-identity --offline`) → C27 → C53 → C64 → review → push → grade.

## 2026-09-07 — chat #40 (secondo blocco) — C26 avviato a chunk: fail-closed AB1 senza infrastruttura proxy (2/4 chunk + preparatorio)

**Tema**: goal `bot-operativo`, blocco A. C26 è il criterio più grande del blocco: eseguito a CHUNK verificati e committati singolarmente, mai big-bang (zero-Q). Metodo: TDD rosso-prima sul chunk che descrive un buco, `/antiban-review` **SICURO** dedicata (quella di C25 copriva altro), WebSearch triangolata sul rischio specifico.

- **Chunk ① — il buco, letto alla fonte**: la guardia AB1 (`launcher.ts:245`) è una congiunzione di QUATTRO condizioni fra cui `proxyInfraConfigured`. Con ZERO proxy configurati la guardia non scatta, `managedProxyEnabled` diventa `false` e `buildProxyLaunchPlan` restituisce `[undefined]` = connessione DIRETTA — anche su sessionDir con cookie LinkedIn e `REQUIRE_PROXY_FOR_AUTH=true`. La configurazione che chiede più sicurezza era quella che degradava per prima. Fix: `requireProxyForAuth` e `sessionHasCookies` **obbligatori** nella firma (coerente con C25: un opzionale qui è fail-open silenzioso), `allowDirectIp` unica uscita esplicita, `failoverChain` opzionale (assente = più severo). Il launcher legge il profilo REALE con `profileHasCookies` (riuso da C23, zero-A) invece di dedurre l'autenticazione dalla presenza di proxy. Rosso-prima 2 FAIL/6 → 13/13. Commit `68fc5ea`.
- **Chunk ② — «0 browser aperti», con deviazione dichiarata**: il VERIFY dice «spy», ma uno spy su `launchBrowser` richiede di mockare `playwright` più l'intera `config` — a quel punto il test osserva i mock, non il codice. Provata la stessa proprietà in forma più stabile: catena REALE (sessionDir temporanea con cookie jar → `profileHasCookies` → throw AB1) + ORDINE nel sorgente (`buildProxyLaunchPlan` precede ogni `launchPersistentContext`, `Camoufox()` e l'import dinamico di `camoufox-js` a `launcher.ts:450`) + guardia anti-regressione sul valore passato dal launcher. Onestà sul metodo: sono casi di forma/regressione aggiunti dopo il fix, senza rosso-prima proprio. 9/9. Commit `46dfeb8`.
- **Preparatorio al chunk ③**: `harnessIdentitySnapshot.ts` → **`src/browser/identitySnapshot.ts`**. Il comando `preflight-identity` è codice di produzione e non può importare da `src/tests/`. Nessun cambio di comportamento, e non l'ho dato per scontato: `harness:identity` rieseguito su **Camoufox reale** (`measures_failed 0`, `external_requests 0`, `exit_code 0`). Commit `3e0c6a1`.
- **Conseguenza voluta da dichiarare nel chunk ③**: dopo il chunk ①, un preflight sul profilo con cookie senza proxy verrebbe BLOCCATO da AB1 — `preflight-identity` è diagnostica su pagina locale (nessun egress), quindi dovrà passare `allowDirectIp: true` in modo esplicito.
- **Osservazione aperta per il chunk ④** (fonte web di oggi: «la rotazione di IP a metà sessione invalida la sessione»): il failover prova proxy in sequenza solo PRIMA di aprire pagine — da verificare esplicitamente, non da assumere.
- **VERIFY complessivo**: tsc 0; `post-modifiche` **258 file / 2493 test exit 0**; `grep -rn "IDENTITY_FORCE_MISMATCH\|BOT_TEST_" src/` = **0** (criterio già verde); working tree pulito, ahead 17, push sospeso per policy di fine blocco.
- **Arresto**: checkpoint di fase a confine pulito prima del chunk ③, che apre un browser reale (in C24 un pezzo simile ha richiesto un planner a 4 tentativi per l'host vero): iniziarlo col margine di contesto rimasto avrebbe rischiato di lasciarlo a metà.


## Blocco 44 — 2026-09-07 (chat #41) — C26 CHIUSO 4/4: `preflight-identity --offline` + diagnosi proxy per account (blocco A 7/9)

**Chunk ③ — `preflight-identity --offline` (commit `54ca0d5`).** Nuovo sottocomando: confronta ciò che la PAGINA espone (`snapshotPage` su `about:blank`) con l'identità dichiarata dal profilo in `.fingerprint.json`; JSON contrattuale `{engine, os, ua, tz, locale, coherent}` più i campi divergenti NOMINATI in `mismatches`; exit 0 coerente, exit 1 divergente. Registrato in `src/index.ts` (case + **`safeCommands`**: è diagnostica read-only, deve poter girare prima che il setup sia completo), `jsonStdout.ts`, `commandHelp.ts`.
- **Design letto alla fonte, non per analogia**: l'harness prende la pagina da `fixtureUrl` di `src/tests/harnessRuntime.ts`, che un comando di produzione non può importare (stessa ragione dello spostamento preparatorio `3e0c6a1`); ma `snapshotPage` costruisce da sé canvas e span su `document.body`, quindi `about:blank` basta. Confermato dal vivo: su `about:blank` restano valide `timeZone`, `userAgent` e `locale`.
- **Verificato DAL VIVO su Camoufox reale**, non solo a test: (a) profilo coerente → `coherent=true` exit 0 (`tz: Europe/Rome`, `locale: it-IT`); (b) `.fingerprint.json` corrotto a `rv:9.9` → `coherent=false` exit 1 con mismatch `userAgent`; (c) profilo configurato senza identità → `IDENTITY_FILE_MISSING` exit 1 **senza aprire il browser**.
- **Il confronto NON è tautologico** (dubbio sollevato prima di fidarsi del verde): il browser mostra il proprio UA reale `rv:135.0`, non quello del file — se lo prendesse dal file il comando non potrebbe rilevare nulla.
- **Buco trovato nella mia prima stesura**: il file identità REALE non persiste `timezone` (Camoufox la deriva da geoip; congelarla la farebbe divergere dal proxy al primo cambio di uscita), mentre la fixture del test la valorizzava — il ramo di confronto sembrava una rete attiva e non lo era. Stesso difetto di C25 («test che conferma invece di verificare»): aggiunto il caso con l'identità nella forma reale.
- **Anti-ban SICURO** con guardia strutturale: `allowDirectIp: true` è la deroga di C26/1 ed è legittima solo finché non esce nulla in rete → un test impone destinazione COSTANTE, nessun flag `--url`, nessuna URL di rete come stringa nel codice.

**Chunk ④ — diagnosi proxy per account (commit `0dc6dff`).** `diagnoseAccountProxy` (`src/proxy/accountProxyDiagnosis.ts`) sostituisce le TRE risposte divergenti alla stessa domanda: booleano secco di `checkProxyHealth` in `preflight-env`, `runFullProxyDiagnostic` GLOBALE che non conosce gli account, `getProxyAsync` che in fondo ritorna `refreshedChain[0]` non verificato. Campi **distinti** `tcp/auth/egress/sticky` + `reason`: un 407 non è «mezzo rotto», è una password sbagliata, e col booleano era indistinguibile da un host morto. Entrambi i consumatori agganciati (`preflight-env` e `config-validate` → `proxyPerAccount`), con test di forma che vieta il ritorno del vecchio booleano.
- **Anti-ban SICURO**: massimo DUE sonde, verso l'endpoint di eco IP già adottato dal repo (`scripts/webrtcLeakCheck.ts:32`), mai verso linkedin.com; è un PRE-flight e non va lanciato mentre una sessione è viva, perché consumerebbe la finestra sticky. `sticky: false` è guasto di primo livello — WebSearch triangolata su 3 fonti indipendenti 2026 (torchproxies · Apify · businessage), **oggettivo**: ruotare l'IP a metà sessione invalida la sessione, la forma corretta è sticky per account 10-30 min (già la configurazione del progetto).
- **Osservazione del blocco 43 CHIUSA verificandola, non assumendola**: entrambi gli usi del failover (`launcher.ts:267` e l'escalation mobile `:819`, che scatta dopo N *launch* falliti) stanno DENTRO il ciclo di lancio, quando non esiste ancora una pagina → nessuna rotazione a metà sessione, **nessun fix necessario**.
- **Trovato di lato e corretto (zero-P)**: `preflightEnv.ts` stampava `account.proxy.server` grezzo nel dettaglio — con credenziali nell'URL sarebbero finite nei log; ora solo host:porta via `maskProxyTarget`, con test che lo verifica.

**VERIFY complessivo**: vitest 10/10 (chunk ③) e 8/8 (chunk ④), i 4 guasti provati contro un proxy HTTP **locale vero** senza mockare il trasporto; tsc 0; lint 0 warning; `post-modifiche` **260 file / 2511 test exit 0** (da 258/2493 a inizio chat); `preflight-env` e `preflight-identity` esercitati dal vivo. Working tree pulito, **ahead 20**, push sospeso per policy di fine blocco A.

**Prossimo**: C27 (LOGIN_REQUIRED graduato in 3 rami espliciti, un solo trattamento per 429/403) → C53 → C64 → review indipendente pre-push → push → grade firmato.

## Blocco 45 — 2026-09-07 (chat #42) — C27 CHIUSO 2/2: la causa del login fallito decide la reazione, da qualunque punto (blocco A 8/9)

**Chunk 2 — innesto (commit `d347f94` browser/risk/repository + test, `d224ff7` core/cli/scripts/cloud + backlog).** La politica pura del chunk 1 (`ecb4039`) ora GOVERNA il runtime: `checkLoginDetailed(page, {accountId})` in `src/browser/auth.ts` restituisce l'esito tipizzato (`logged-in` / `logged-out` / `two-factor` / `unknown` con causa / `throttled` 429|403) e il booleano `checkLogin` resta un wrapper per i 18 caller che non decidono; `src/risk/loginFailureHandler.ts` (NUOVO) è l'UNICO punto che applica la reazione — quarantena + pausa 60' sul logout (un incident, non due), `pauseAutomation` con backoff + proxy in cooldown + sticky rilasciato sul throttling, pausa 15' senza incident sull'esito ignoto.
- **Punti innestati**: canary (`workflowEntryGuards.ts`), job runner ×3 (rotazione, avvio, probe del feed), listener voyager del launcher (UNA volta per sessione: un burst di 429 era N incident con backoff ×2^n fino a 24h), più i gemelli `audit.ts` (site-check) e `companyEnrichment.ts`. `LINKEDIN_PRE_THROTTLED` e `LOGIN_MISSING` non vengono più emessi dal job runner. `probeLinkedInStatus` riconosce il **403** (prima → `SESSION_EXPIRED` → quarantena: il gemello del bug 429).
- **2FA per-account anche su `default`**: `repositories/system.ts` — solo l'id ASSENTE scrive il flag globale; `default`, nome reale dell'account runtime, ha la sua chiave `account_quarantine:default`. `incidentManager.quarantineAccount` passa `undefined` per l'incidente non attribuibile: il fail-safe globale è invariato. I messaggi dicono il comando esatto (`bot.ps1 unquarantine --account <id>`).
- **`recordSuccessfulAuth` toglie la pausa `LOGIN_REQUIRED`** e solo quella (async, 5 caller attesi): una pausa più lunga fusa sotto lo stesso motivo dalla monotonia di `setAutomationPause` si riconosce dalla scadenza e resta.
- **Anti-ban SICURO** (6 domande): stessa navigazione al feed, pause solo più conservative (429 → ≥180' con backoff invece di quarantena/pausa breve; esito ignoto → 15' invece di «procedi»), nessuna nuova azione, volumi invariati; il listener voyager resta al SOLO 429 (un 403 su singola API è spesso legittimo → falso positivo da 3 ore).
- **Residuo dichiarato**: `releaseStickyProxy` cancella la mappa in memoria; al lancio successivo `getStickyProxy` ri-matcha il proxy persistito della stessa settimana (`proxyManager.ts:750-790`), come già faceva il listener storico — il rilascio serve al failover, non cambia l'IP dell'account.

**VERIFY**: `loginRequiredGraduato.vitest` **22/22** (rosso-prima exit 1 con handler assente; chiave `account_quarantine:default=true` e flag globale assente ASSERITI; handler provato contro i repository REALI su `sync_state` finto), `workflowEntryGuards.vitest` +2 casi (429 dal canary → `pauseAutomation('HTTP_429_RATE_LIMIT', …, 180)`, `quarantineAccount` mai chiamato), 5 test esistenti riallineati alla nuova semantica di `default`; `post-modifiche` **261 file / 2536 test exit 0** (da 2522); tsc 0; lint 0 warning; madge 0 circolari; `security:scan` 0 su 949 file. Working tree pulito, **ahead 24**, push sospeso per policy di fine blocco A. Backlog: `LINKEDIN_IMPLEMENTATION_LIST.md` item 21 e `workflow-architecture-hardening.md` «separare LOGIN_MISSING da rate limit/403/timeout/proxy» spuntati.

**Prossimo**: C53 (contratto di `identity-init`) → C64 → review indipendente pre-push → push → grade firmato.

## Blocco 46 — 2026-09-07 (chat #43) — C53 CHIUSO: la cartella di sessione di un account ha UNA sola risposta (blocco A 9/9)

**Premessa del criterio corretta alla fonte prima di scrivere codice (zero-M).** Il contratto diceva «rosso-prima: il sottocomando non esiste (`src/index.ts`, letto)»: era obsoleto, `identity-init` esiste da C23 (`src/index.ts:670`, `cli/commands/identityInit.ts`, `commandHelp.ts:165`, `jsonStdout.ts:31`). Il testo firmato NON è stato modificato — un edit dentro il criterio ha spostato il `contract_hash`, l'hook di drift lo ha rilevato e l'edit è stato revertito nello stesso turno (digest tornato a `a781f9ae…`, provato dal `grade` successivo). Il **delta vero** di C53 era la risoluzione UNICA della cartella, e il rosso-prima reale è stato `resolveSessionDir is not a function` — **5 FAIL / 8**.

**Il difetto**: la regola «account → cartella» era ripetuta in 5 punti indipendenti (`accountManager` ×2 nel profilo di default, `launcher.ts:233`, `companyEnrichment.ts:292`, `webrtcLeakCheck.ts:111`). Due risposte diverse alla stessa domanda significano `login` che scrive i cookie in una cartella e un altro flusso che ne apre un'altra: per LinkedIn sono due dispositivi sullo stesso account. Dentro `runLoginCommand` la divergenza era già in nuce — il browser partiva su `selectedAccount.sessionDir` e `recordSuccessfulAuth` scriveva la baseline di freschezza su una lettura separata.

- **`resolveSessionDir(accountId?)` (NUOVA, `accountManager.ts`)**: unico punto che legge `config.sessionDir`, ritorna sempre un path assoluto e replica esattamente la semantica di `getAccountProfileById(...).sessionDir` **senza ricorsione** — i due fallback del profilo default chiamano lei, e lei non passa da loro (usa i profili configurati).
- **`launcher.ts`**: `options.sessionDir ?? resolveSessionDir(options.accountId)`. Chi non dichiara la cartella non cade più su una lettura locale di `config`.
- **`runLoginCommand`**: una sola `const sessionDir` per il lancio E per `recordSuccessfulAuth`. `companyEnrichment` e `webrtcLeakCheck` passano dalla funzione (valore invariato: entrambi restano sull'account corrente).
- **Decisione (zero-C.10)**: i **14 call site** che passano `account.sessionDir` (path di `send-invites`: jobRunner, syncSearchService, salesNavCommands, audit, doctor, listActions, …) NON sono stati riscritti — il valore è identico per costruzione e riscrivere 14 punti in area anti-ban è rischio senza guadagno; scartato «migrare tutto» perché la regressione possibile (una cartella diversa) è proprio il danno che il criterio vuole evitare. L'uguaglianza non resta assunta: è ASSERITA da un invariante che gira su ogni profilo runtime.
- **Anti-ban SICURO** (6 domande): nessuna navigazione, timing o volume toccato. Il rischio reale — cambiare cartella e perdere i cookie — escluso alla fonte: `config.sessionDir` è già assoluto (`domains.ts:85` → `env.ts:161-166`), quindi `path.resolve` è idempotente, e nessun caller di `launchBrowser` passa `accountId` senza `sessionDir` (l'unico che passa accountId, `preflightIdentity.ts:72-73`, passa entrambi).

**VERIFY**: `identityInitSessionDir.vitest.ts` (NUOVO, 10/10 exit 0) — test AST via `typescript` (`PropertyAccessExpression`: stringhe e commenti non contano) che il resto di `src/` ha **0 letture** dirette di `config.sessionDir` e che dentro `accountManager` la lettura sta SOLO fra le righe di `resolveSessionDir`; invariante per ogni profilo; contratto del comando (cartella pulita → creata + path stampato; cartella con cookie → exit 1, mtime invariata, 0 scritture; `--new-session` → cartella nuova + riga di configurazione; cartella INESISTENTE creata dal codice VERO `createBrowserIdentity`, con `os` = host asserito). `post-modifiche` **exit 0 CATTURATO = 262 file / 2547 test** (da 261/2537); tsc 0; lint 0 warning; madge **0 circolari** (l'import nuovo `launcher → accountManager` non ne introduce). **DAL VIVO su `dist`**, exit di node catturato: cartella inesistente → creata, `os: windows` = host, UA major 135 = binario `135.0.1-beta.24`, exit 0 · stessa cartella → `created: false`, exit 0 · profilo VERO `data/session` (24 cookie) senza flag → **exit 1** `IDENTITY_PROFILE_HAS_COOKIES` e `.fingerprint.json` lì **non esiste** (0 scritture verificate dopo) · `--new-session` → `base-20260907-215402` con `configHint` che stampa `SESSION_DIR=<path>` e il comando successivo.

**Errore mio nel turno (tracciato)**: ho dichiarato «post-modifiche exit 0» leggendo l'exit di `tail` in una pipe, mentre il lint stava fallendo (2 warning `no-non-null-assertion` con `--max-warnings 0`) e la suite non era nemmeno partita. Corretto nello stesso turno; da lì ogni gate è stato misurato con l'exit del comando vero. È la seconda occorrenza della stessa classe (blocco 43): la pipe che maschera l'exit non va usata sui gate.
