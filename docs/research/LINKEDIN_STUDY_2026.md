...
3. **Integrity/iniezione (basso per noi)** — BrowserGate/integrity-check penalizzano l'**iniezione** DOM, non la lettura. Il nostro `page.evaluate` legge e `clickLocatorHumanLike` usa input nativo: allineato. Un drift-metric sui selettori è l'early-warning giusto, **ma la copertura reale è più stretta di quanto questo studio assumesse** (corretto 2026-08-05): l'unica implementazione viva è `assessSelectorModelDegradation` (`selectors/learner.ts:205-215`), che gira **solo** con `!dryRun && autoRollback` e **solo** sulle label promosse dall'ultima run di learning. Fuori da quel perimetro — `audit.ts`, scraper, inviteWorker-connect — oggi il drift **non lo misura nessuno**. (`uiFallback.measureSelectorDrift`, citata qui in origine, era una seconda implementazione senza un solo chiamante: rimossa in `e917137`.)
