...
~~A12.~~ ☁️ **CHIUSA NEL CLOUD** (Ultraplan / Claude Code web, 2026-06-13 — decisione utente "considerala chiusa nel cloud"). Pacing per-account implementato + revisionato nella sessione cloud; review anti-ban avvenuta lì. **NON verificata localmente** (al 2026-06-13T17:25 il codice non era nel repo: `gh pr list` + `git log --all` = solo `c4ca43e` docs, nessun branch/PR di implementazione). Design + infra-test (sotto) restano come riferimento se servirà riconciliare la PR cloud col branch locale.

A12 (design — riferimento storico). `[Anti-ban][EPIC]` **Correlazione budget multi-account** (scheduler.ts:519-533). Design verificato da workflow 11-agenti (2026-06-13, 1 APPROVED / 2 REVISE con controesempi).
...
~~A11-2.~~ ✅ **FATTO** (commit d9c738f, 2026-06-13): ring buffer in-memory (ultimi 50) dei soli eventi CRITICI + replay-on-subscribe in `liveEvents.ts` → la dashboard recupera il live-feed al reconnect (eventi replayed marcati `_replayed:true`). **Cambio vs design originale** (verifica alla fonte): NON `outbox_events` — è sink-based worker→cloud (`claimPendingOutboxEvents`/lease), semantica errata per replay SSE multi-client; e la persistenza durevole degli eventi critici **esiste già** (outbox→cloud + audit_log + Telegram A11-1), il gap reale era solo il live-feed SSE. +5 test, conta-problemi exit 0 (179f/1775t).
